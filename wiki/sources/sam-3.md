...
### 既存 open-vocabulary 検出器（GLIP / OWLv2 / gDino 等）の弱点

CLIP テキストエンコーダ + 検出ヘッドというパターン（**GLIP [[entities/glip]]** / **[[entities/grounding-dino|Grounding DINO]]** / OWLv2 / LLMDet）は存在したが：
...
| **検出器** | DETR ベース（[[sources/detr]] / [[entities/detr]] → (M)DETR/Deformable DETR/Plain DETR の系譜）、fusion encoder で画像 ↔ プロンプトトークンを融合 → DETR decoder + マスクヘッド | コンセプトプロンプトから画像全フレームの instance マスクを生成 |
