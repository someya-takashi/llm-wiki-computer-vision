...
related: [[knowledge-distillation]], [[vision-transformer]], [[knn-evaluation-protocol]], [[masked-image-modeling]], [[foundation-model]], [[weakly-supervised-pretraining]], [[gram-anchoring]]
sources: [[sources/dino-emerging-properties-in-self-supervised-vit]], [[sources/dinov2-learning-robust-visual-features-without-supervision]], [[sources/dinov3]]
updated: 2026-05-24
---
...
- **BYOL** (Grill et al., 2020): student の最後に **predictor**（追加 MLP）を付け、teacher（momentum encoder）の出力に MSE で近づける。predictor + momentum + BN の組合せで崩壊を防ぐ。
...
BERT の流儀を画像に持ち込む系統。入力の一部を隠し、隠した部分を再構成または予測させる。ViT との親和性が高い。詳細: [[masked-image-modeling]]。

- **BEiT** (Bao et al., 2021): 事前学習済み dVAE が出力する離散トークンを予測。
- **MAE** (He et al., 2022): 画素を 75% マスクし、軽量 decoder で再構成。
