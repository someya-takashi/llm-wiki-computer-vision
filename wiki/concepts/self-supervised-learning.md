...
- **BYOL** (Grill et al., 2020, [[sources/byol]]): student の最後に **predictor**（追加 MLP）を付け、teacher（momentum encoder）の出力に MSE で近づける。predictor + EMA ターゲットの組み合わせが必須で崩壊を防ぐ。負例なしで SimCLR を 5 pt 上回り（74.3% vs 69.3%）、「負例は必須か」という問いに「否」と答えた先駆的論文, [[sources/byol]]): student の最後に **predictor**（追加 MLP）を付け、teacher（momentum encoder）の出力に MSE で近づける。predictor + EMA ターゲットの組み合わせが必須で崩壊を防ぐ。負例なしで SimCLR を 5 pt 上回り（74.3% vs 69.3%）、「負例は必須か」という問いに「否」と答えた先駆的論文。
