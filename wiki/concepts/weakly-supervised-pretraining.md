...
- **DFN** ([[sources/dfn]] / [[entities/dfn]]) (Fang et al., Apple + University of Washington, ICLR 2024, arXiv:2309.17425): **「データ・フィルタリング・ネットワーク」パラダイムを提唱**、データセット自体を訓練対象とせず、**小さな CLIP（HQITP-350M で訓練した ViT-B/32 + OAI-Init + COCO/Flickr/IN ファインチューン）で DataComp 12.8B から上位 15% を選別し DFN-2B を生成**。さらに 42B プールから **DFN-5B** を生成。**DFN-5B で訓練した ViT-H/14 が ImageNet ゼロショット 84.4%** で 2023 年末当時の SOTA（LAION-2B / DataComp-1B / OpenAI WIT を凌駕）。**3 つの非自明な発見**: (1)「フィルタ性能 ≠ ImageNet 性能」（IN -30% でもフィルタとして同等）、(2)「データ品質が決定的」（CC12M を少量汚染するだけでフィルタ性能崩壊）、(3)「バイナリ分類器より CLIP フィルタ」。**「データセット設計はモデル設計と同じツールを使える」** という主張で、データ中心 AI ムーブメントの代表。**[[entities/qwen2-vl|Qwen2-VL]] / [[entities/qwen2-5-vl|Qwen2.5-VL]] の 675M ViT 初期化に DFN-CLIP が採用**（[[entities/qwen3-vl|Qwen3-VL]] は SigLIP-2 に切り替え）。**HQITP-350M は Apple 内部非公開だが、公開データ（CC12M+CC3M+SS15M）で再現可能**を §4.2 で実証。**DFN モデル公開**（apple/DFN5B-CLIP-ViT-H-14-378 等）、**DFN-2B データセット公開**
