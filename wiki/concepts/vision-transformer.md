...
related: [[self-supervised-learning]], [[knowledge-distillation]]
...
**画像を 16×16 のパッチに切り分け、各パッチを「単語」のように扱って Transformer に流し込むだけの画像分類モデル**。Dosovitskiy ら（Google Brain）が 2020 年に発表（"An Image is Worth 16x16 Words", ICLR 2021）。それまで CV を支配していた CNN（畳み込みニューラルネット）の代替として、十分な事前学習データがあれば CNN 並みかそれ以上の精度を出せることを示した。
