...
| **[[entities/deepseek-ocr\|DeepSeek-OCR]]**（DeepSeek-AI, 2025, [[sources/deepseek-ocr]]） | Wei et al. | **SAM-base (80M) を「Visual Perception 成分」として活用** — 高解像度文書画像を window attention で処理 → 2 層 ConvNet で 16× 圧縮 → CLIP-large へ。**DeepEncoder (約 380M) の前段**として、segmentation 用途ではなく **文書 OCR の効率的視覚エンコーダ**に再利用される新しい応用例。OmniDocBench で 800 トークン未満で MinerU2.0 (6790 トークン) を凌駕 |
...
- [[sources/foundational-models-vision-survey]] — Awais et al. の CV 基盤モデル survey で SAM を「軸 2 視覚プロンプト型」の中核として位置付け、医療・追跡・リモートセンシング・モバイル等の SAM 適応も体系的に整理
