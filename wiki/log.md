...
## [2026-08-28] ingest | BiomedCLIP: a multimodal biomedical foundation model pretrained from fifteen million scientific image-text pairs

- 取り込み: `raw/papers/BiomedCLIP_ a multimodal biomedical foundation model pretrained from fifteen million scientific image-text pairs.md`（Zhang, Xu, Usuyama, Xu ほか / Microsoft Research + University of Washington + Providence Genomics, 2023, arXiv:2303.00915）
- 作成: [[sources/biomedclip]], [[translations/biomedclip]], [[entities/biomedclip]], [[entities/pmc-15m]]
- 更新: [[concepts/medical-foundation-model]], [[concepts/weakly-supervised-pretraining]], [[entities/clip]], [[entities/medsiglip]], [[entities/eva-x]], [[overview]], [[index]]
- 図: `raw/assets/biomedclip/fig{1,2,3,4,5}.png`（5 点）
- メモ:
  - **ユーザー指示により Supplementary Note も全訳**（補足表 1〜10 込み）。References のみ除外。**原典 markdown には図3 が欠落していた**（埋め込みは図1・2・4・5 の 4 枚のみ）ため、5 点すべて arXiv e-print の tarball から生成した。**ar5iv 変換の欠損は「一部の図だけ落ちる」形でも起こる**という教訓。
  - **前回の MedGemma ingest の直後に来たのが効いた**。作成したばかりの [[concepts/medical-foundation-model]] に**4 類型目（公開データによる領域特化事前学習）**を足す形で自然に収まり、[[entities/medsiglip]] との対比が本 ingest の中心になった。
  - **着想が単純で強い**: 「論文の図には必ずキャプションが付いている。それはそのまま画像–テキスト対である」。PMC-OA の 440 万件から機械的に抜くだけで **15,282,336 対**——MIMIC-CXR（37.7 万）の**約 40 倍**。既存の医療画像–テキストデータの 3 限界（**私的 / 小さい / 胸部 X 線に偏る**）を一度に解いている。データセット単独で [[entities/pmc-15m]] を `entity_kind: dataset` で切り出した（[[entities/wit-400m]] / [[entities/lvd-142m]] / [[entities/sa-1b]] の先例に倣う）。
  - **本論文で最も価値があったのは Supplementary Note のアブレーション**。**「検証で良くなったのに下流で悪くなった」ケースが 3 件も記録されている**: (1) 解像度 384 は検証 R@1 +1.7pt だが**下流ゼロショット分類は 75.52 → 70.37 と −5.2pt**、しかも訓練時間 1.92 倍。(2) バッチ 64k は検証 +3.3pt だが「利得は下流に転移しない」（著者の説明: 極端に大きなバッチはより多くの訓練データを要する、CLIP は 4 億対だが PMC-15M は 1,500 万対）。(3) ImageNet 初期化は検証でランダム初期化にわずかに負けるが下流で安定。**3 つとも検証 retrieval の R@1 を信じて選んでいたら間違えていた。** [[sources/l2rw]] の「Random ベースラインが既存手法を全部倒した」と同型の教訓（**代理指標の改善が本番の改善を意味しない**）として sources に明記した。さらに図3 を読むと **BiomedCLIP (GPT) 版が RSNA と TCGA-TIL で BERT 版を上回る**——検証 retrieval では BERT が 10pt 圧勝なのに逆転しており、**4 件目の乖離**になる。
  - **本論文の目玉は「多様性による正の転移」**。RSNA 肺炎検出で**ラベル 10% だけで完全教師ありの放射線科特化 BioViL を上回る**。しかも論文は「BiomedCLIP の放射線関連画像は BioViL の MIMIC-CXR より多くはなく、対はおそらくはるかにノイジー」と自ら明記して**「放射線データを多く見たから勝った」という解釈を潰している**。この誠実さは評価したい。[[entities/eva-x]] に「反例: 多様性が特化を上回る場合」の節を追加し、**「精度で勝つための特化」から「効率で勝つための特化」へと特化の意義が移った**と位置づけた（EVA-X の 6M という規模の優位は揺らがない）。
  - **PubMedCLIP の失敗が本 ingest で最大の発見**かもしれない。ROCO（放射線 8 万対）で汎用 CLIP をファインチューニングした結果、PMC-15M の検索で **R@1 が 1.00% と汎用 CLIP の 8.48% の 1/8** に落ちている。**汎用性を失ったうえに新領域も広くカバーできない**という最悪の組み合わせ。これを [[concepts/medical-foundation-model]] に「補論: ドメイン適応の失敗例」として立て、**成功している 2 路線がどちらもこの罠を別の方法で回避している**ことを整理した——[[entities/biomedclip]] は「データを 2 桁増やして完全に置き換える」、[[entities/medsiglip]] は「元データを残して医療を 2% だけ混ぜる」。**危ないのはその中間**（元の分布を壊すには十分だが新領域を覆うには足りない量）。前回 MedGemma で「2% という控えめな配合」に感心したが、**その慎重さの理由が本論文で裏付けられた**形になった。
  - **[[entities/medsiglip]] との対比表が本 ingest の中心**。同じ「医療の CLIP 系画像エンコーダ」なのに**ほぼ全項目で正反対**（ゼロから vs SigLIP 出発 / 医療 100% vs 医療 2% / InfoNCE vs sigmoid / 完全再現可能 vs 重みのみ / 汎用性を捨てる vs 保持 / 224 vs 448）。どちらも成功しているのは**目的が違う**から——MedSigLIP は汎用 MLLM の視覚エンコーダとして機能する必要があるが、BiomedCLIP は生物医学の中で広く使える表現を作ればよい。両ページに相互の対比節を置いた。
  - **「1,500 万の生物医学画像」という語感と実態のずれを明記した**。図1B を読むと**最頻の画像型は統計図表・グラフ・チャート・表・フローチャート・システム概要図**といった汎用の科学図解で、放射線・病理はその下に来る。正の転移が効いたのは事実だが**有効な医療画像の実数はもっと小さい**。加えて論文の図は「**発表に値するほど典型的か珍しいもの**」に偏り、キャプションは臨床レポートほど網羅的に所見を書かない（その論文が言いたい 1 点だけ）。**公開 = 偏りがない、ではない**という形で [[concepts/medical-foundation-model]] の補論に書いた。
  - **プライバシー保護の代理の実験は論文の本文が言うより弱い**。本文は recall 88.80（肺の陰影）と 95.04（無気肺）だけを挙げるが、図5B を読むと **F1 は全 4 条件で 12〜42% と低く**（無気肺は recall 95.04 に対し F1 25.20 で適合率が壊滅）、**胸水は recall 25.35**、**心拡大では BiomedCLIP 6.99 に対し汎用 CLIP が 36.24 と 5 倍良い**。論文は心拡大の負けには言及して「論文のキャプションは臨床レポートほど包括的でない」と説明しており、そこは誠実。図から全体像を読み取って sources に表で載せた。
  - **本文と図の数値が食い違う**のを 1 件記録。本文は検索を「上位 5 件は 77% 超、上位 1 件は 56% 超」と書くが、図2A のラベルは R@5 86.30 / R@1 69.60。どちらも「77% 超 / 56% 超」を満たしはするがそう書く理由がなく、**評価設定が異なる可能性**がある。図のラベル値を採ることにして sources に注記した。MedGemma 1.5 の RadGraph F1 の食い違いに続き、**同一論文内・世代間の数値の突き合わせは毎回やる価値がある**。
  - **[[entities/clip]] と [[concepts/weakly-supervised-pretraining]] にも波及した**。「汎用 CLIP は PMC-15M の検索で R@1 8.48%」という数字は、**「CLIP は汎用に見えて実は Web 分布に特化していた」ことを裏側から示す**。WSL パラダイムが領域を越えて移植可能であること、ただし**「対になったデータをどこから取るか」を領域ごとに解き直す必要がある**ことを概念ページに追記した。
  - ⚠ **作業中に [[concepts/weakly-supervised-pretraining]] の frontmatter を正規表現の誤りで一時的に壊した**（`sources: [...]` の最初の `]` が `[[sources/clip]]` の内側にマッチした）。git から元の行を取得して修復済み。**frontmatter を正規表現で編集するときは wikilink の `]]` に注意する**。
  - **dangling link（今後の ingest 候補）**: **LLaVA-Med**（BiomedCLIP を視覚エンコーダに使う直接の後続、優先度高）、**PLIP**（医療 Twitter の病理 CLIP、LC25000 で BiomedCLIP を上回る）、**BioViL / GLoRIA / ConVIRT**（CXR 特化 VLP の系譜）、**PubMedBERT**、**Med-Flamingo**、**BiomedGPT-B**、**Med-PaLM M**、**ELEVATER**。既存の未消化候補（Med-Gemini、Med-PaLM、RadFM、GMAI-MMBench、Virchow、PolyPath、MAIRA、CoCa-CXR、BioViL-T、InternImage、EfficientNet 系、CoAtNet、Swin V2、SimMIM、DeiT、U-Net、DeepLab v3+、PSPNet、Focal Loss、MAML 等）はそのまま。

## [2026-08-31] ingest | Early-Learning Regularization Prevents Memorization of Noisy Labels (ELR)

- 取り込み: `raw/papers/Early-Learning Regularization Prevents Memorization of Noisy Labels.md`（Sheng Liu, Jonathan Niles-Weed, Narges Razavian, Carlos Fernandez-Granda / New York University, **NeurIPS 2020**, arXiv:2007.00151）
- 作成: [[sources/elr]], [[translations/elr]], [[concepts/learning-with-noisy-labels]]
- 更新: [[concepts/example-reweighting]], [[concepts/semi-supervised-learning]], [[overview]], [[index]]
- 図: `raw/assets/elr/fig{1..7}.png`（7 点）
- メモ:
  - **ユーザー指示により Appendix A〜I をすべて全訳**。**Appendix A（550 行）は線形モデルにおける早期学習と暗記の理論解析**で、命題 3・定理 4・命題 5・定理 6・補題 7-10 とその証明からなる密な数式の連鎖。数式は LaTeX のまま保持し地の文を訳した。翻訳ファイルは 936 行と本 wiki 最長になった。
  - **entity ページは作らず概念ページを新設**（[[sources/l2rw]] で確立した先例に倣う）。ELR はモデルでなくアルゴリズムなので `entity_kind` のいずれにも当てはまらない。**受け皿として [[concepts/learning-with-noisy-labels]] を新設**した。既存の [[concepts/example-reweighting]] に統合する選択肢もあったが、**本論文は自らを「既存手法はクリーンな事例を検出するが、我々は正則化で行く」と位置づけており、重み付けとは別の枝**であるため分離が正しいと判断した。ラベルノイズに限った話題を新ページに移し、example-reweighting は「何を根拠に重みを決めるか」に集中させた。
  - **本 ingest の核心は「早期学習 → 暗記」という動力学**。図1 が論文のすべてを語っており、交差エントロピーでは暗記された事例の割合が 0.05 → 0.87 まで単調増加し、テスト精度がエポック 30 のピーク 0.83 から 0.65 まで落ちる。ELR では暗記が 0.03 で平坦に保たれる。
  - **理論パートで最も面白かったのは「深層性ではなく高次元性が暗記の原因」という主張**。暗記の条件が **$p/n>1-\Delta/2$** であり、**ロジスティック回帰でも起こる**。証明の骨格も理解しやすい——早期は「係数が全事例で似た値になるので勾配が大多数（正しい事例）の平均方向を指す」、後期は「$\theta$ が $\mathbf{v}$ と相関すると直交方向を指し始め、次元が高ければ直交方向が暗記に十分なだけ存在する」。**定理 6 が特に美しく**、「ノイズラベルの暗記」を「線形分離可能性」に帰着させて **Schläfli の定理**で数え上げ、$2\cdot\mathbb{P}[\mathrm{Bin}(n,\Delta/2)\leq n-p]$ で抑える。
  - **手法の設計で最も転用可能な知見は Appendix C**。「モデル出力と目標の KL ダイバージェンスにペナルティ」という自然な代替が**失敗する**理由を勾配解析で示している——**ELR は符号が「$\mathbf{t}_c$ と目標の他の成分の差」で決まるのに対し、KL は「$\mathbf{t}_c$ とモデル出力 $\mathbf{p}_c$ の差」で決まるため目標そのものに過適合する**。「**目標に近づける**」と「**目標が指す方向へ押す**」は違う、という形で sources と概念ページの両方に書いた。これが④ラベル修正との根本的な違いでもある。
  - **Appendix D の「自己参照のループ」も記録した**。temporal ensembling だけを行い交差エントロピーで訓練すると**目標自体がノイズに過適合する**。正則化項は暗記を防ぐと同時に**自分自身の入力である目標を守っている**。Appendix H の「$\beta=0$ にすると 38% まで崩壊」も同じ話で、移動平均は「あれば良い」ではなく必須の部品。
  - **[[sources/l2rw]] との対比が本 ingest 最大の収穫**。同じ NYU/Uber 周辺の 2 年差の 2 論文が、同じ問題に**外部の基準（クリーンな検証セット 15 枚）vs モデル自身の過去**という対照的な道具で当たっている。L2RW は追加データを要求する代わりに収束の理論保証を得て、ELR は追加データ不要な代わりに「早期学習が実際に起こる」という経験的前提に乗る——**そして本論文の理論パートはまさにその前提を正当化するためにある**。**ELR は L2RW を Related Work で明示的に議論の対象外に置いている**（「クリーンな部分集合の利用可能性を仮定する」手法として）のも記録した。訓練コストも 3× vs ほぼ 1× と対照的。
  - **[[sources/l2rw]] の警告が本論文に跳ね返る点を批判的視点として書いた**。L2RW は「**人工的なノイズ設定では強い正則化をかけるだけで驚くほどうまくいく**」と警告し、乱数の重みというベースラインで既存手法を全部倒してみせた。**ELR は文字どおり正則化項であるだけに、この疑いは軽くない**。ところが ELR には「目標を乱数にする」「$\mathbf{t}$ を定数にする」といった対照がない（Appendix D は目標推定を外した対照であって、正則化項の中身が効いているかを切り分ける対照ではない）。**「クリーンな検証セット不要」と言いつつハイパラ調整には検証セットを使っている**点も指摘した。
  - **訓練時間が隠れた売り**。ELR **1.1h** / ELR+ **2.3h** に対し DivideMix 5.4h、Co-teaching+ 4.4h。**DivideMix の 1/5** で ELR 単体でも CIFAR-10 非対称 40% が 90.12（次点 83.55 に約 8pt 差）。「損失関数に項を 1 つ足すだけ」という単純さがそのままコストに現れている。
  - **[[concepts/semi-supervised-learning]] との合流を両ページに明記した**。ELR+ の 4 要素（temporal ensembling / 重み平均化 / 2 ネットワーク / mixup）は**すべて半教師あり学習由来**で、DivideMix に至ってはノイズラベル問題そのものを半教師あり学習として定式化する。**高ノイズほど半教師あり技法の寄与が支配的**（40% では各要素 1〜2pt だが 80% では mixup 単体で 10pt 以上）。「ラベルが足りない」と「ラベルが汚い」は方法論のレベルで同じ問題になりつつある。
  - **論文の正直さも記録した**。ELR+ vs DivideMix の比較で**評価プロトコルの差を明示**し（他手法は「訓練中の最高検証精度」、ELR+ は別途取った検証セット）、同じ土俵に揃えた ELR+ ∗ の列も併記している。ILSVRC12 転移で DivideMix に 4.9pt 負けることも明記。
  - ⚠ **原典の欠落を 1 件記録**: ar5iv の変換で**筆頭著者 Sheng Liu の名前が著者欄から脱落している**（謝辞の「SL was partially supported by...」、参考文献 [^25]、コードリポジトリ `shengliu66/ELR` から確認できる）。§1 の貢献の箇条書きも脱落している。翻訳の冒頭に正しい著者リストと欠落の事実を明記した。**ar5iv 変換の欠損が「図」「疑似コード」に続いて「著者名」でも起こった**ことになる。
  - **図は 7 点すべて複数パネルの合成**。tarball の `images/*.png`（個別パネル）を LaTeX の figure 環境の並び順に従って組み合わせた。fig1 は 2 行 3 列（CE の clean/false/test + ELR の clean/false）、fig5 は 3 行 2 列（KL 正則化の λ = 1/5/10 × clean/false）。
  - **dangling link（今後の ingest 候補）**: **"Understanding deep learning requires rethinking generalization"**（Zhang et al., ICLR 2017 — 「深層ネットワークは何でも暗記できる」の原典、**優先度が高い**）、**"A closer look at memorization in deep networks"**（Arpit et al., ICML 2017 — 早期学習現象の初出）、**DivideMix**（本論文の最大の競合）、**Co-teaching**、**MentorNet**（L2RW / ELR 双方の比較対象）、**Mean Teacher** / **Temporal Ensembling**（ELR+ の部品の原典）、**mixup**（本 wiki の複数ページに登場するが未取り込み）、**GCE / SL**。既存の未消化候補（LLaVA-Med、PLIP、BioViL、Med-Gemini、InternImage、EfficientNet 系、Focal Loss、MAML、U-Net、DeepLab v3+ 等）はそのまま。

## [2026-08-31] ingest | DenseNets Reloaded: Paradigm Shift Beyond ResNets and ViTs (RDNet)

- 取り込み: `raw/papers/DenseNets Reloaded_Paradigm Shift Beyond ResNets and ViTs.md`（Donghyun Kim, Byeongho Heo, Dongyoon Han / NAVER Cloud AI + NAVER AI Lab, **ECCV 2024**, arXiv:2403.19588）
- 作成: [[sources/rdnet]], [[translations/rdnet]], [[entities/rdnet]], [[concepts/skip-connection]]
- 更新: [[concepts/convolutional-neural-network]], [[entities/convnext]], [[entities/swin-transformer]], [[entities/hrnet]], [[overview]], [[index]]
- 図: `raw/assets/rdnet/fig{1..7}.png` + `figA{1,2,3}.png`（10 点）
- メモ:
  - **ユーザー指示により Appendix 0.A〜0.I を全訳**（454 行）。**0.I（287 行）がパイロット研究の詳細**で、本論文の主張の根拠である「15,000 個のランダムネットワークで連結 vs 加算を比べる」実験の設定と結果表を含む。0.F（転移性）と 0.G（生成モデル）も本文にない知見だった。
  - **原典 markdown の欠落が今回も**: ar5iv には画像が 4 枚しか埋め込まれておらず、**本論文の看板図であるアーキテクチャ図（図1）が欠落**していた。tarball から取得。さらに **§5.2〜5.3 の本文が二重に出力されている**（ar5iv 変換のバグ）ため翻訳では重複を除いた。**ar5iv の欠損はこれで「図」「疑似コード」「著者名」「本文の重複」の 4 種類目**。
  - **本 ingest の中心は [[sources/convnext]] との対比**。同じ手続き（現代レシピでベースラインを引き直し、要素を 1 つずつ変えて段階表を作る）を **ResNet ではなく DenseNet に適用**している。しかも **RDNet は ConvNeXt のブロック設計をそのまま借りている**（LN・depthwise・7×7・活性化を減らす）ので、「ConvNeXt の成果を土台にして、**ConvNeXt が触らなかった軸——ショートカットの型——を変えた**」という素直な系譜になる。両ページに対比表を置いた。
  - **[[concepts/skip-connection]] を新設したのが本 ingest 最大の構造的変更**。wiki には「加算 vs 連結」という軸が存在せず、しかも**これは CNN と Transformer を横断する**（ViT も Swin も ConvNeXt も内部は加算）。加えて既存の [[entities/hrnet]] の多解像度融合、U-Net 系の skip、[[entities/mae]] の skip がすべてこの軸に乗るのに、束ねるページがなかった。**「勾配を運ぶ / 特徴を運ぶ / 解像度を運ぶ」という 3 目的が同じ名前で呼ばれている**という整理を軸に据えた。
  - **検証方法が本論文で最も価値がある**。単一のモデルを 2 つ作って比べるのではなく、**設計空間全体（深さ・幅・活性化 5 種・正規化 2 種・カーネル 4 種・ブロック型 3 種）から無作為抽出した分布同士を比べる**。これは RegNet 由来の「設計空間を population として評価する」手法で、**「たまたまチューニングが上手くいっただけ」という交絡を潰す**。[[sources/l2rw]] が「乱数ベースラインを置け」と警告した問題への別方向からの回答として位置づけた。
  - **ただし論文の主張と自身の発見に緊張関係がある**ことを批判的視点に書いた。パイロット研究の差は**平均約 1pt で標準偏差（約 2）より小さい**。しかも Appendix 0.I.1 で著者自ら「**データ拡張が加算と連結の差を縮める。特に AdamW で stochastic depth を用いた場合は加算がより恩恵を受ける**」と記録しており、表K の $\mathcal{E}$ 空間の Sto. Depth（57.5 vs 57.5）と RandErase（58.1 vs 58.1）では**差が消えている**。「連結が本質的に強い」と「近代的レシピが差を埋める」は両立しにくい。
  - **要旨の主張が部分的にしか成立していない点も指摘した**。要旨は「メモリ効率を高める」と書くが、**RDNet-T 4.1GB vs ConvNeXt-T 2.7GB / Swin-T 2.6GB で ResNet 系には負けている**。表1 の進行でも訓練メモリは 3.9 → 3.1GB としか改善しない。**DenseNet 比では正しいが ResNet 系比では成立していない。**
  - **b1 レイテンシの差が異常**（RDNet-T 7.4ms vs BiFormer-S 51.6ms で 7 倍）。FLOPs やパラメータ数では見えない差で、[[sources/convnext]] の「A100 で Swin 比 +49% スループット」と同じ論点のさらに極端な例。**ただし b128 では差が縮む**（175.2 vs 183.7）ので「速度優位は小バッチ・低レイテンシ領域に偏る」と明記した。
  - **Appendix 0.F の転移性が最も驚いた**。**RDNet-T（24M）が iNaturalist-2018/2019 で DeiT-III-L（304M）を上回る**（77.0 vs 75.6、81.2 vs 79.3）。13 倍のパラメータ差を覆している。長尾分類で連結型が強いのは「特徴の再利用が少数クラスの学習を助ける」という DenseNet 本来の主張と符合する。
  - **Appendix 0.H の OOD 頑健性も重要**。論文が明示する通り「**RDNet が競合より ImageNet-1K で低い精度を示す場合でさえ、高い OOD スコアを達成する**」。RDNet-L は ConvNeXt-L に対し IN で +0.5 なのに Avg Shift で **+2.3**、Sketch で **+4.4** 開く。**ImageNet top-1 だけでモデルを選ぶことの危うさ**を示す例として記録した。
  - **「skip があるから他の要素が要らない」という定石を疑う議論を概念ページに立てた**。DenseNet は「接続パターンが似ているので Stochastic Depth は不要」とされていたが、実際には **0 → 0.15 で +1.2%p**（ロードマップのどの単一ステップより大きい）。これは [[entities/nfnet]] の「残差接続があるので正規化は必須」を疑った例、[[entities/hiera]] の「ViT の特殊モジュールは必須」を疑った例と同型で、**アーキテクチャの一部が別の一部の要否を暗黙に決めているという思い込み**の一般的な形として整理した。
  - **CKA 解析の扱いは慎重にした**。「RDNet は各層で異なる特徴を学ぶ、ConvNeXt は中盤の層同士が似る」という観察は面白いが、**なぜ多様性が高いと良いのかは論じられていない**ので、解釈（「加算は同じ空間に足すので似通いやすく、連結は新しいチャネルを足すので分化しやすい」）には ⚠ を付けた。
  - **Transformer 側での連結ショートカットは未検証**という空白を概念ページに明記した。論文は CNN のみを扱い、この方向を今後の課題として明示もしていない。ただし Appendix 0.G で **WGAN の生成器を連結型に置き換えて FID 27.79 → 25.37** を示しており、分類以外への一般化可能性は示唆されている。
  - **dangling link（今後の ingest 候補）**: **ResNet**（He et al., CVPR 2016 — 加算ショートカットの原典で、**本 wiki に依然として専用ページがない**。優先度が高い）、**DenseNet**（Huang et al., CVPR 2017 — 連結の原典）、**U-Net**（skip の原典）、**Identity Mappings in Deep Residual Networks**、**Deep Networks with Stochastic Depth**、**RegNet**（「設計空間を population として評価する」手法の原典）、**DeiT-III**、**HorNet / VAN / NAT / SMT / MogaNet / BiFormer / SLaK / InceptionNeXt / RevCol**（2022〜2024 の最新アーキテクチャ群）。既存の未消化候補（Zhang et al. 2017「Understanding deep learning requires rethinking generalization」、DivideMix、LLaVA-Med、PLIP、Med-Gemini、InternImage、EfficientNet 系、Focal Loss、MAML、DeepLab v3+ 等）はそのまま。

## [2026-09-01] ingest | InceptionNeXt: When Inception Meets ConvNeXt

- 取り込み: `raw/papers/InceptionNeXt_ When Inception Meets ConvNeXt.md`（Weihao Yu, Pan Zhou, Shuicheng Yan, Xinchao Wang / NUS + SMU + Sea AI Lab + Skywork AI, **CVPR 2024**, arXiv:2303.16900）
- 作成: [[sources/inceptionnext]], [[translations/inceptionnext]], [[entities/inceptionnext]]
- 更新: [[concepts/convolutional-neural-network]], [[concepts/skip-connection]], [[entities/convnext]], [[entities/rdnet]], [[entities/swin-transformer]], [[sources/rdnet]], [[overview]], [[index]]
- 図: `raw/assets/inceptionnext/fig1.png`〜`fig4.png`（4 点）
- メモ:
  - **翻訳の範囲**: Abstract + §1-5 + **Appendix A・B を全訳**。Appendix はハイパーパラメータ表（表9・表10）と Grad-CAM の定性結果のみで合計 ~25 行しかないため、ユーザーに確認せず全訳した（ルーチンの判断）。References のみ除外。
  - **ar5iv 変換の欠陥（本 wiki で 5 回目）**: 原典 markdown に**画像が 1 枚（Grad-CAM の 1 コマ）しか埋め込まれておらず、看板図である図1（精度 vs スループット）・図2（4 ブロックの図解）・図3（FLOPs 比較）がすべて欠落**していた。arXiv e-print tarball（2303.16900, 503KB）から `figures/inceptionnext_first_figure.pdf` / `InceptionNet_block.pdf` / `two_types_of_conv.pdf` を pymupdf でレンダリング。図4 の Grad-CAM は 5 行（原画像 / resnet / swin / convnext / inceptionnext）×4 列を手動で組み合わせた。**fig1 と fig2 は Read で目視検証済み**（fig1 の各点の座標、fig2 の 4 ブロックの構成要素を確認）。
  - **ページ構成はユーザー確認済み**。「FLOPs と実速度の乖離」の概念ページを新設するか尋ねたところ、**「entity までの 3 点」**（新設しない）との回答。その論点は [[concepts/convolutional-neural-network]] の「カーネルサイズ」節に `#### 大カーネルの隠れたコスト` として収め、[[overview]] の「主要技術トピック」にも 1 項目立てた。**この軸は現在 9 つの entity ページ（convnext / rdnet / nfnet / hrnet / maxvit / convnext-v2 / dinov3 / sa-co / deepseek-ocr / sam-3）に散らばっており、将来ページ数が増えたら概念ページ化を再提案する余地がある**。
  - **本論文の中心は 1 つの数字**: 「ConvNeXt-T は ResNet-50 と同程度の FLOPs（4.5G vs 4.1G）なのに A100 訓練スループットは 575 vs 969 img/s と約 60%」。**ConvNeXt 論文自身は A100 スループットを Swin との比較でしか出しておらず、同 FLOPs の ResNet-50 とは並べていない**——比較対象の選び方だけで論点が可視化された例として [[entities/convnext]] に節を立てた。
  - **[[entities/rdnet]] との関係が本 ingest 最大の収穫**。前回 ingest した RDNet とまったく同じ診断（ConvNeXt の 7×7 depthwise が実機で遅い）から出発し、**別の処方**（RDNet はショートカットの型、InceptionNeXt はトークン混合器）を出している。RDNet の表4 に **InceptionNeXt-T が 132ms と表中最速のレイテンシ**で載っており（RDNet-T は 82.8 / 175ms）、**精度で RDNet、速度で InceptionNeXt**という住み分けになる。**両者が変えた軸は直交しており、組み合わせは誰も試していない** ⚠ ——この空白を [[entities/convnext]] / [[entities/rdnet]] / [[sources/inceptionnext]] の 3 箇所に記録した。
  - **[[concepts/skip-connection]] に第 3 の軸を追加した**。同ページはこれまで「加算 vs 連結」（合流のさせ方）だけを扱っていたが、本論文の **MetaNeXt** が**ショートカットの「本数」**という軸を出している。MetaFormer はトークン混合器用と MLP 用で 2 本持つが、ConvNeXt は 1 本しかない。そして **MetaNeXt-Attn（1 本のブロックに自己注意を入れた版）は top-1 3.9% で収束しない**。同ページの主題である「アーキテクチャの一部が別の一部の要否を暗黙に決めている」の**逆向きの依存**（skip を減らすと X が置けなくなる）として整理した。⚠ ただし 1 論文の 1 実験（等方的、22M、IN-1K）で、失敗の原因は論文でも分析されていないので留保を明記。
  - **論文自身が「なぜ大きいモデルでは効かないか」を説明している**のが誠実で有用。depthwise の計算量は $\mathcal{O}(C)$、MLP は $\mathcal{O}(C^2)$ なので、チャネル数が増えると計算が MLP に支配される。実際スループット改善は **Atto +219% → Tiny +57%** と減衰する。**この手法はエッジ・軽量向けである**という適用範囲の限定を sources / entities / overview の 3 箇所に書いた。
  - **BN vs LN の判断が [[sources/convnext]] と逆になっている**点を [[concepts/convolutional-neural-network]] の正規化節に追記した。ConvNeXt は「近代化を全部済ませた後なら LN がわずかに良い」（81.41 → 81.47）と結論したが、InceptionNeXt は **LN の方が精度 +0.1（82.4 vs 82.3）だと確認したうえで、訓練スループットが 20% 落ちる（901 → 721）ため BN を採用**した。**目的関数が違えば同じ実験から逆の結論が出る**という素直な例。
  - **アブレーションから読める設計の勘所**を entity ページに表で残した。帯状カーネルのどちらか一方を除くと **−0.4（最大の劣化）**で、これが受容野を広げる本体。一方 **3×3 の除去は −0.3 で済みしかも速くなる**——著者自ら「速度重視なら落としてよい」と示唆している。帯状カーネルは **11 で頭打ちで 13 では下がる**（論文は「最適化に起因するかもしれず、構造的な再パラメータ化で解決できる可能性がある」と留保）。
  - **⚠ マーカーの解消**: [[index]] と [[sources/rdnet]] の「HorNet / VAN / NAT / SMT / MogaNet / BiFormer / SLaK / **InceptionNeXt** / RevCol = 2022〜2024 年の最新アーキテクチャ群（本 wiki 未取り込み）」から InceptionNeXt を外し、sources/rdnet 側には ingest 済みである旨を追記した。
  - **批判的視点として書いたこと**: (1) スループット測定のバッチサイズが「GPU が収容できるまで減らす」可変設定で、1 条件あたり 1 数字しか出ていない（RDNet が示した通りこの種の比較は条件で大きく動く）、(2) 精度の絶対値では RDNet-T 82.8 に負けており、価値は**精度あたりの速度**にある、(3) **BatchNorm を選んだ副作用が未検証**——[[entities/nfnet]] が整理した BN の既知の問題（バッチ依存、対比学習での情報漏洩）を踏まえると自己教師あり学習への適性は射程外、(4) RDNet との直接比較がない（時期的に不可能）。
  - **dangling link（今後の ingest 候補）**: **MetaFormer Is Actually What You Need for Vision**（Yu et al., CVPR 2022 — MetaNeXt の親であり、同じ筆頭著者。**本論文の理解に最も直結するので優先度が高い**）、**PoolFormer**（MetaFormer の具体化）、**ShuffleNet V2**（「全チャネルを畳み込みに通す必要はない」の出どころ）、**FocalNet**（InceptionNeXt-T と同点 82.3）、**Inception / GoogLeNet**（本論文の名前の由来だが本 wiki に専用ページがない）。既存の未消化候補（ResNet、DenseNet、U-Net、Identity Mappings、Stochastic Depth、RegNet、DeiT-III、HorNet / VAN / NAT / SMT / MogaNet / BiFormer / SLaK / RevCol、InternImage、EfficientNet 系 等）はそのまま。
