...
- [[sources/maxvit]] — MaxViT（Tu et al., Google Research + UT Austin, **ECCV 2022**）。**「窓の中で attention（局所）」+「等間隔に間引いた画素どうしで attention（大域）」を逐次に並べるだけで、線形計算量のまま第 1 層から大域視野を得る**手法。[[sources/swin-transformer|Swin]] の shifted window が窓をずらして大域性を*間接的に*得たのに対し、**grid attention で*直接*得る**。**Max-SA（multi-axis self-attention）**: 入力を $(\frac{H}{P}\times\frac{W}{P}, P\times P, C)$ に切れば**窓内 attention（block）**、$(G\times G, \frac{H}{G}\times\frac{W}{G}, C)$ に切れば**格子点間 attention（grid, dilated 大域）**。$P=G=7$（Swin に合わせる）で**局所と大域の計算量が完全に釣り合い、双方とも線形**。**Swin の attention モジュールとパラメータ数・FLOPs が完全同一の drop-in 置換**でありながら、**マスキング・パディング・cyclic shift が一切不要**（einops の rearrange 2 つで書ける）。**MaxViT ブロック = MBConv → block attention → grid attention**（MBConv の depthwise conv が **CPE として働き明示的な位置埋め込みが不要**に）。**主要結果**: IN-1K のみ 512² で **86.70%**（当時の通常訓練 SOTA）/ **IN-21K → 1K で MaxViT-B 88.38%（CoAtNet-4 をパラメータ 43%・FLOPs 38% で +0.28 上回る）** / IN-21K MaxViT-XL **88.70%** / JFT-300M **89.53%** / COCO 896² で MaxViT-B **53.4 AP**（**MaxViT-S が約 40% 少ない計算量で Swin-B / UViT-B 超え**）/ AVA 美観評価 PLCC **0.745**（多重解像度の MUSIQ 超え）/ GAN で **FID 30.77・IS 22.58 を 18.6M（HiT 32.9M の約半分）**で達成。**アブレーション**: grid を block に置換（**同パラメータ・同 FLOPs**）しても -0.13〜-0.23 → **大域性そのものの純粋な寄与**を切り出した対照実験。MBConv 除去で最大 -0.97、ブロック順序は **C-BA-GA が最良**（MBConv を後ろにすると -0.54〜-0.60）だが**生成タスクでは逆順 GA-BA-C が良い**、**逐次 vs 並列で並列が -0.98〜-1.63 と大差**（HiT 等の並列設計への反証）。**弱点**: **スループットが Swin/ConvNeXt の半分以下**（MaxViT-T 349.6 vs Swin-T 755.2 / ConvNeXt-T 774.7）、IN-1K のみではパラメータ効率も劣る、最高スコアは非公開の JFT-300M 依存、検出比較は解像度が横並びでない（896² vs 1280×800）、**「単純さ」の主張は相対的**（1 ブロックに MBConv + SE + 2 種 attention + FFN×2）。**原典の誤植**: Appendix 表13 の MaxViT-L @384 は 84.40 とあるが本文表2 では 86.40（後者が正）。

- [[sources/nfnet]] — NFNet「High-Performance Large-Scale Image Recognition Without Normalization」（Brock, De, Smith, Simonyan / DeepMind, **ICML 2021**）。**「BatchNorm を完全に取り除いても、むしろ速く・強くなる」ことを初めて実証した論文**。**BN の 3 つの実践的欠点**（計算が高価 / 訓練・推論の乖離 / **ミニバッチ内の事例の独立性を壊す**）と **4 つの恩恵**（残差分岐のダウンスケール / 平均シフト除去 / 正則化 / 大バッチ訓練）を分解し、後者を個別の仕掛けで代替する。**Normalizer-Free の骨格**: $h_{i+1}=h_i+\alpha f_i(h_i/\beta_i)$、$\beta_i$ は $\mathrm{Var}(h_{i+1})=\mathrm{Var}(h_i)+\alpha^2$ から**解析的に予測**、遷移ブロックで分散リセット、**SkipInit**（ゼロ初期化スカラー）、**Scaled Weight Standardization** で平均シフト除去。**中核は AGC（Adaptive Gradient Clipping）**: 勾配ノルムそのものではなく **「勾配ノルム / 重みノルム」の比**（= 1 ステップでの相対的な重み変化量）で、**層単位でなくユニット単位**にクリップ。$\lambda=0.01$（バッチ 1024-4096）、**最終線形層には適用しない**、**バッチが大きいほど $\lambda$ を小さく**。LARS 等「正規化オプティマイザ」の**緩和版**（上限のみ課し下限は課さない）。**アーキテクチャ**: GELU 付き SE-ResNeXt-D から、**グループ幅 128 固定**（小グループ幅は FLOPS を減らすが計算密度が落ち実速度が上がらない）+ **深さ $[1,2,6,3]\times N$**（ResNet の非一様な深さ増加は最適でない）+ **幅 $[256,512,1536,1536]$**（第 3 ステージに容量を足すのが最良）+ **$3\times3$ グループ化 conv を 1 つ追加**。**設計指針が独特**——理論 FLOPS でも推論レイテンシでもなく**実機（TPUv3/V100）の訓練レイテンシ**を最適化対象にした。スケーリングは**深さと解像度のみ**（幅はスケールしない）。**主要結果**: **NFNet-F6+SAM で IN-1K 86.5%（追加データなし当時 SOTA）** / **NFNet-F1 が EffNet-B7 と同じ 84.7% を 8.7× 速い訓練で**（158.5ms vs 1397.0ms）/ **JFT-300M → IN で NFNet-F4+ 89.2%**（転移学習で当時最高、**1.86k TPUv3-core-days** と Meta Pseudo Labels の 22.5k の 1/12）/ **NF が BN を転移で全サイズ・全解像度で約 1 ポイント上回る** / **同一アーキの BN 版より精度が高く訓練が 20-40% 速い**（F4/F5 は BN では訓練が不安定）。**アブレーションの率直さ**: アーキテクチャ変更の寄与（+0.9/+0.4）より**データ拡張の寄与（+2.3/+3.0）の方がはるかに大きい**。**Appendix の知見**: 大規模事前学習では **weight decay をゼロから始めて軽く増やせ**（ViT 論文と正反対）、Adam より SGD、**大きいモデルを短く訓練する方が効率的**、SAM は**上昇ステップにバッチの 20% だけ使えば同性能**（訓練時間 +20-40%）、否定的結果（Appendix E）としてステージごとの異質な設計・DCT ダウンサンプリング・$1\times1$ のグループ化はいずれも不発。**弱点**: **パラメータ効率が悪い**（F1 132.6M で 84.7% vs EffNet-B7 66.0M で同 84.7%、著者も明記）、最高値は**非公開 JFT-300M 依存**、**AGC の $\lambda$ は依然調整が要る**、部品と調整項目が多い（$\alpha,\beta,\lambda,\epsilon,\gamma$）、**attention を一切試していない**（著者自ら「採用すれば改善しうる」と明記）。**BN 排除は主流にならなかった**（翌年 [[entities/convnext]] は LN への置換という単純な解を採用）が、**BN の欠点の整理は広く引用され続けている**。
- [[sources/hrnet]] — HRNet「Deep High-Resolution Representation Learning for Visual Recognition」（Wang et al., Microsoft Research Asia ほか, arXiv:1908.07919 → **IEEE TPAMI 2021**、CVPR 2019 会議版の大幅拡張）。**LeNet-5 以来の「解像度を落としてから復元する」設計規則を捨てたバックボーン**。$1/4$〜$1/32$ の 4 本の畳み込みストリームを**並列に維持**し、その間で**双方向の多解像度融合を 8 回繰り返す**。姿勢推定・セグメンテーション・検出という **position-sensitive（位置に敏感な）タスク**に特化した設計。**3 つのヘッド**: HRNetV1（高解像度のみ、CVPR 版）/ HRNetV2（4 解像度連結）/ HRNetV2p（V2 + 特徴ピラミッド、検出用）。**主要結果**: COCO pose test-dev **75.5 AP**（V1-W48）、val で ImageNet 事前学習なしの W32 が **73.4 AP** と SimpleBaseline ResNet-152（72.0）を半分以下の GFLOPs で上回る / Cityscapes val **81.1 mIoU**（V2-W48）で **PSPNet と同パラメータ・約 1/3 の GFLOPs（696 vs 2018）で +1.4** / PASCAL-Context **56.2 mIoU**（+OCR）/ COCO 検出は Faster/Cascade R-CNN・FCOS・CenterNet・Mask R-CNN・HTC の全枠組みで ResNet/ResNeXt 超え、**特に AP_S（小物体）で顕著**。**アブレーションの知見**: 融合回数が効く（1 回 70.8 → 3 回 71.9 → **8 回 73.4 AP**）/ 融合を乗算にすると **54.7 AP と壊滅** / ダウンサンプルは学習可能でないと駄目（双線形化で −0.8 AP）/ **4 本を最初から並走させると逆に下がる**（72.5 < 73.4、低解像度の初期低レベル特徴は有用でない）/ 最低解像度のヒートマップは **AP 10 未満**。**理論**: 並列多解像度畳み込み ≒ group convolution、多解像度融合 ≒ 通常の畳み込みの多分岐全結合形式 — この等価性が「V1 は出力チャネルの一部を捨てている」＝ V2 の根拠になり、次元だけ揃えた対照実験 **HRNetV1h** で裏付けられた。**ImageNet 分類での優位は小さい（ResNet-152 21.2% → HRNet-W96 21.0% err.）** ことが「解像度維持は分類のための工夫ではない」ことを示す。**限界**: 多分岐構造は最適化コストが高く推論時間がフレームワーク依存（PyTorch では遅く MXNet では同等）/ SSL・言語接続は射程外 / 解像度は $1/4$ 止まり。ICCV 2019 の COCO キーポイント検出チャレンジで**ほぼ全参加者が採用**。
- [[sources/l2rw]] — L2RW「Learning to Reweight Examples for Robust Deep Learning」（Ren, Zeng, Yang, Urtasun / Uber ATG + University of Toronto, **ICML 2018**, arXiv:1803.09050）。**訓練事例の重みを「損失の大きさ」ではなく「小さなクリーンな検証セットの勾配方向との一致度」で決めるメタ学習アルゴリズム**。**問題設定の指摘が鋭い**: 従来の重み付けには「hard 優先」（AdaBoost / hard negative mining / **Focal Loss** — 損失が大きい＝少数クラス）と「easy 優先」（self-paced learning / Reed bootstrapping — 損失が大きい＝ノイズ）という**正面から矛盾する 2 つの流儀**があり、不均衡かつノイジーなデータでは**どちらの前提も誤る**。さらに「**不偏なテストセットの定義なしに訓練セットバイアス問題を解くのは本質的に ill-defined**」と論じる。**手法**: 二重レベル（bilevel）最適化を 1 ステップ近似に畳み、$\epsilon$ をゼロ初期化 → 仮の 1 ステップ更新 → 検証損失を $\epsilon$ で微分 → 整流 → バッチ内で総和 1 に正規化（**この正規化がメタ学習率を打ち消し「追加ハイパラゼロ」を成立させる**）。重みは $w_i \propto \max\{\nabla G^\top \nabla f_i, 0\}$ ＝ **検証勾配と訓練勾配の内積の整流**。MLP 展開では入力の類似度 × 勾配方向の類似度の積の和（Eq.12）。**実装**は backward-on-backward（2 階自動微分）で任意アーキテクチャに適用可、**訓練時間 3×**。**理論**: 検証損失の単調減少（補題1）と収束レート $O(1/\epsilon^2)$（定理2、**SGD と同じ**）。**主要結果**: MNIST 4-9 不均衡 199:1 で誤り率 **約 3.7%**（Baseline 約 18%）/ CIFAR-10 UniformFlip 40% noise **86.92**（MentorNet 76.6）/ CIFAR-100 **61.34** / BackgroundFlip CIFAR-10 **86.73** ・CIFAR-100 **59.30**（オラクルのノイズ比を知る S-Model+Conf+ES+FT の 55.75 超え）。**非自明な知見**: ⚠**整流ガウス乱数の「Random」ベースラインが UniformFlip で 86.06 と既存手法を全部上回った**（著者自ら「驚くべき」と記述、乱数の重みが強い正則化として働く仮説）—— ただし BackgroundFlip では 69.51 と崩れる / **クリーンな検証は 15 枚で足り 100 枚で頭打ち**、著者の解釈は「**データ源ではなく正則化子**」/ **早期停止が不要**（訓練精度が約 60% ＝ クリーン率で頭打ちになり、ノイズを一切暗記しない）/ 混同行列で背景クラスの列が消える / **ノイズ 0% ではベースラインをわずかに下回る**（検証セットのサブサンプルバイアス、論文が正直に記載）。**限界**: 訓練 3× / **不偏でクリーンな検証セットが手に入る前提**（バイアスの判断を 15 枚のキュレーションに集約しただけとも読める）/ MNIST・CIFAR 規模に留まり ImageNet 規模は未検証 / 大規模事前学習時代には重心が「事例ごとの重み」から「データセット規模の選別」（[[entities/dfn]]）へ移った。**原典の誤り**: Appendix B Eq.29 の $F(\theta)$ の係数は $1/M$ でなく $1/N$ であるべき（結論に影響なし）。
- [[sources/medgemma]] — MedGemma（Google Research + Google DeepMind, 2025, arXiv:2507.05201）。**[[entities/gemma-3]] と [[entities/siglip]] のアーキテクチャを 1 ミリも変えず、医療データの「混ぜ方」だけ**で作った医療視覚–言語基盤モデル。**3 段構え**: (1) **視覚エンコーダ強化** — SigLIP-400M を 3,300 万超の医療画像–テキスト対（うち **3,260 万が病理パッチ = 98%**）で微調整、**元の WebLI を残し医療を 2% の重みで混合** → これが [[entities/medsiglip]] になる。(2) **デコーダ事前学習** — 医療 **10%** の重み、約 5 エポック、**医療テキストは追加しない**。(3) **後訓練** — 蒸留 + RL、**マルチモーダルは SFT より RL の方が汎化する**ため全て RL。**非自明な前処理**: CT の HU を 3 ウィンドウ（骨/肺 2250/-100、軟部 350/40、脳 80/40）で **RGB チャネルに写像**。**896² と 448² は同一重み**（位置埋め込みのダウンサンプルのみ）で、医療タスクは 448² で足りたため公開 MedSigLIP は 448²。**主要結果**: MedQA **64.4**（Gemma 3 4B 50.7、**+13.7**）/ 27B は TTS 込みで **87.7**（74.9）/ CheXpert OOD **48.1**・CXR14 OOD **50.1** で **Gemini 2.5 Pro を 11〜13pt 上回る** / EyePACS **64.9**（Gemma 3 4B 14.4）/ 微調整で CRC100k **32.8 → 94.5**、EHRQA **86.3 → 93.6**（RL、誤り約 50% 減）/ 汎用性能はほぼ維持（Global MMLU Lite は **55.5 > 54.5** と上回る）。**最も重い結果は Appendix A の放射線科医評価**: 306 症例で「81% が同等以上の臨床判断」だが、裏返すと**約 18% は AI に従うと患者管理を誤り、しかも異常例の方が悪い**。**非盲検・評価者 1 名**という限界も論文が明記。**限界**: MedXpertQA は 27B で 25.7 と **o3 の 54.6 の半分以下**（飽和したベンチマークが差を隠している）/ **訓練データの中核が非公開で再現不可能** / 皮膚科では Gemini 2.5 Pro に負ける（**Web に画像が豊富なモダリティでは特化の利得が小さい**）/ 27B マルチモーダルは 27B テキストより弱い。
- [[sources/medgemma-1-5]] — MedGemma 1.5（Google Research + Google DeepMind, 2026, arXiv:2604.05081）。**アーキテクチャを一切変えずに** 3D CT/MRI・病理 WSI・解剖学的局在化・経時 CXR 解析を 4B に載せた。効かせたのは**前処理と [[entities/gemma-3]] の 128K 長コンテキスト**。**3D**: 軸位断 2D スライス列に落とし**最大 85 枚 = 21,760 視覚トークン**（プロンプトと出力を含めて **32K 以内**に収めるため）、超過時は z 軸方向に等間隔サンプル。**CT の HU → RGB 写像は各チャネルの理由が明示的**（R(-1024,1024) 全体の形態 / **G(-135,215) 軟部 — 緑は輝度への寄与が最大だから** / B(0,80) 脳実質・出血）。**MRI は物理単位がないのでウィンドウ処理せず volume 単位の min-max 正規化で R=G=B**。**WSI**: 5x で組織マスク（HSV）→ **倍率を確率的に選択**（5x/10x/20x ≒ 1/3 ずつ）→ 896² 非重複パッチ**最大 126 枚 = 32,256 トークン**、**空間順序を保存**。**訓練**: MedSigLIP を**凍結**し言語デコーダのみ更新、**モダリティ別の補助教師**（CT/MRI/病理）を蒸留に追加。**主要結果**: WSI 病理レポート **2.2 → 49.4**（外部 SOTA PolyPath 49.8 に肉薄）/ 局在化 IoU **3.1 → 38.0**（**外部 SOTA CoCa-CXR 30.7-34.4 を上回る**）/ 3D MRI 51.3 → **64.7** / 3D CT 58.2 → **61.1** / EHRQA 67.6 → **89.6** / MedQA 64.4 → **69.1**。⚠ **2.2 と 3.1 はほぼランダム出力の水準で、「改善」ではなく「まったくできなかったことができるようになった」**と読むのが正確。**CT-RATE（OOD の 3D CT）で 4B が 26.9 に対し Gemini 3.0 Flash は 8.5** と 3 倍以上。**[[entities/qwen3-vl]] 4B との対比が論文の見どころ**: テキストの医学知識は Qwen3-VL が勝つ（MedQA 76.8 vs 69.1）が、**すべての視覚タスクで MedGemma 1.5 が勝つ**（EyePACS 76.8 vs 41.9、局在化 38.0 vs 8.7）——論文は「明確に異なる設計思想」と表現。**限界**: **SLAKE が 72.3 → 59.8（-12.5）と大きく退行**、PubMedQA -5.8、**MMLU Pro は Gemma 3 4B 比 -9.8pt** で論文自ら「**4B クラスの明確なトレードオフ**」と認める / スライス等間隔サンプルなので**薄いスライスの小病変は原理的に落ちうる**（「本当の 3D」ではない）/ **代表図（図3）に幻覚が写っている**（放射線科医が「実質的な肝内胆管拡張は明らかでない」と不同意）/ CT-RATE は 1 検査あたり **18 回の問い合わせ**が必要 / **RadGraph F1 が 1.0 の論文と食い違う**（21.9 vs 29.5、プロンプト標準化のため世代をまたぐ比較に注意）/ **系統的な人間評価がない**。
- [[sources/biomedclip]] — BiomedCLIP（Zhang, Xu, Usuyama, Xu ほか / Microsoft Research + University of Washington + Providence Genomics, 2023, arXiv:2303.00915）。**論文の図とキャプションはそのまま画像–テキスト対である**という着想で、PubMed Central の **440 万件の公開全文論文から 15,282,336 対（[[entities/pmc-15m|PMC-15M]]）**を抽出し、[[entities/clip]] を生物医学向けに作り直した。**MIMIC-CXR（37.7 万対）の約 40 倍**で、既存の医療画像–テキストデータの 3 限界（**私的 / 小さい / 胸部 X 線に偏る**）を一度に解く。**訓練データが完全公開・再現可能**なのが最大の特徴。**手法**は CLIP の InfoNCE と二塔の骨格をそのままに、領域に合わない部品だけを差し替える: **GPT-2 → PubMedBERT**（R@1 64.53 → 69.03）、**BPE → WordPiece**（BPE は専門用語を細断する）、**文脈 77 → 256**（PMC キャプションの 90% を覆う、→ 73.50）、ViT-S/M/B で単調改善（69.45 → 71.85 → **73.50**）。**主要結果**: PMC-15M 検索で **txt2img R@1 69.60 / R@5 86.30**（汎用 CLIP は **8.48**、⚠**PubMedCLIP は 1.00 と汎用 CLIP の 1/8**）/ ゼロショット分類で PCam 73.5・RSNA 79.0・LC25000(Colon) 93.0 / VQA-RAD トークン F1 **73.13 で 562B の Med-PaLM M（62.06）を 11pt 上回る**。**本論文の目玉は正の転移**: **RSNA 肺炎検出でラベル 10% だけで完全教師ありの放射線科特化 BioViL を上回る**。しかも論文は「BiomedCLIP の放射線関連画像は BioViL の MIMIC-CXR より多くはない」と明記して「放射線データを多く見たから」という解釈を潰し、**非放射線科の画像型を含む多様性そのものが効いた**と結論する。**Supplementary Note のアブレーションが最良の部分**: ⚠**検証で良くなったのに下流で悪くなったケースが 3 件**（解像度 384 は検証 +1.7pt だが下流ゼロショット分類 **75.52 → 70.37** かつ訓練時間 1.92 倍 / バッチ 64k は検証 +3.3pt だが「利得は下流に転移しない」/ ImageNet 初期化は検証でランダムに負けるが下流で安定）——**代理指標を信じて選んでいたら 3 つとも間違えていた**。**限界**: **PMC-15M の最頻の画像型は統計図表・グラフ・表など汎用の科学図解**で医療画像ではない / 論文の図は「発表に値するもの」に偏り、キャプションは臨床レポートほど網羅的でない / **画像の半分が複合図**でそのまま 1 対扱い / 引用箇所（citances）未使用 / **プライバシー保護の代理は論文が言うより弱い**（F1 は全条件 12〜42%、**心拡大では汎用 CLIP に 5 倍負ける** 6.99 vs 36.24）/ 本文と図の数値が食い違う（本文「R@5 77% 超 / R@1 56% 超」vs 図2A の 86.30 / 69.60）。**LLaVA-Med は BiomedCLIP を中核の視覚エンコーダとして使っている**。
- [[sources/elr]] — ELR「Early-Learning Regularization Prevents Memorization of Noisy Labels」（Sheng Liu, Jonathan Niles-Weed, Narges Razavian, Carlos Fernandez-Granda / New York University, **NeurIPS 2020**, arXiv:2007.00151）。**深層ネットワークはノイズラベルで訓練すると、まずクリーンなラベルを学び（早期学習）、後から誤ったラベルを暗記する**という動力学を理論と手法の両面から扱った論文。**理論**: 2 つのガウス分布の混合という最も単純な設定で、**この現象が線形モデル（ロジスティック回帰）でも起こることを証明**した（定理 1）。早期学習が成功するのは「$\theta$ が正しい方向 $\mathbf{v}$ と相関していない間は係数が全事例で似た値になり、勾配が**大多数を占める正しい事例の平均方向**を指す」ため。暗記が起こるのは「$\theta$ が $\mathbf{v}$ と相関すると勾配が直交方向を指し始め、**次元が十分高ければ直交方向が暗記を許すのに十分なだけ存在する**」ため。暗記の条件は **$p/n>1-\Delta/2$**——つまり**「深層だから暗記する」のではなく「高次元だから暗記する」**。定理 6 は「ノイズラベルの暗記」を「$S_+$ と $S_-$ の線形分離可能性」に帰着させ **Schläfli の定理**で数え上げる。**手法**: 損失に項を 1 つ足すだけ。$\mathcal{L}_{\text{ELR}}=\mathcal{L}_{\text{CE}}+\frac{\lambda}{n}\sum_i\log(1-\langle\mathbf{p}^{[i]},\mathbf{t}^{[i]}\rangle)$（$\mathbf{t}$ はモデル自身の過去出力の移動平均 = temporal ensembling）。**同じ 1 つの項が事例の種類で自動的に逆向きに働く**——クリーンな事例では消えかけた交差エントロピーの寄与を大きく保ち、誤ラベル事例では暗記の方向を打ち消す。**サンプル選択と違い「どちらの事例か」を判定する必要がない**。**主要結果**: CIFAR-10 非対称 40% で **ELR 90.12**（次点 Forward 83.55、**約 8pt 差**）/ 対称 80% で ELR⋆ **80.69**（SL 68.12）/ CIFAR-100 対称 20% で **74.21** / Clothing1M で ELR+ **74.81**（DivideMix 74.76）/ WebVision top1 **77.78**（DivideMix 77.32）。**訓練時間が隠れた売り**: ELR **1.1h** / ELR+ **2.3h** に対し DivideMix 5.4h、Co-teaching+ 4.4h（**DivideMix の 1/5**）。**Appendix C が設計の核心**: KL ダイバージェンスによる正則化は失敗する——ELR は符号が「$\mathbf{t}_c$ と**目標の他の成分**の差」で決まるのに対し、KL は「$\mathbf{t}_c$ と**モデル出力** $\mathbf{p}_c$ の差」で決まるため**目標そのものに過適合する**。**「目標に近づける」と「目標が指す方向へ押す」は違う**。**Appendix D**: temporal ensembling だけでは目標自体がノイズに過適合する（自己参照のループは正則化なしには壊れる）。**Appendix H**: $\beta$・$\lambda$ への感度は低いが、**$\beta=0$（移動平均なし）にすると 38% まで崩壊**する。**限界**: 線形と非線形のギャップが未解決 / ILSVRC12 転移で DivideMix に 4.9pt 負け / **⚠「クリーンな検証セット不要」と言いつつハイパラ調整には検証セットを使っている** / **⚠ [[sources/l2rw]] が置いた「Random ベースライン」相当の対照がなく、正則化効果と真の頑健化を切り分けていない**。**原典の欠落**: ar5iv 変換で**筆頭著者 Sheng Liu の名前が著者欄から脱落**している（謝辞の「SL」、コード `shengliu66/ELR` から確認できる）。
- [[sources/rdnet]] — RDNet「DenseNets Reloaded: Paradigm Shift Beyond ResNets and ViTs」（Donghyun Kim, Byeongho Heo, Dongyoon Han / NAVER Cloud AI + NAVER AI Lab, **ECCV 2024**, arXiv:2403.19588）。**[[sources/convnext|ConvNeXt]] が ResNet に対してやったことを DenseNet に対してやった論文**。ただし主張はもう一段深く、**「加算ショートカットより連結ショートカットの方が本来強い」**という命題を検証する。**問題設定が鋭い**: ResNet 以降ほぼすべて（ConvNeXt も ViT も Swin も）が**加算ショートカット** $\mathbf{X}+f(\mathbf{X}\mathbf{W})$ を共有しており、「Transformer 対 CNN」の対立の下で**残差学習という同じ土台を全員が共有していた**。DenseNet 系だけが**連結** $[\mathbf{X}, f(\mathbf{X}\mathbf{W})]$ を採る。**検証方法が本論文最大の特徴**: **Tiny-ImageNet で 15,000 個超のランダムネットワークをサンプル**し、パラメータ数/FLOPs/メモリを揃えて**ショートカットだけを加算/連結で入れ替える**（深さ・幅・活性化 5 種・正規化 2 種・カーネル 4 種・ブロック型 3 種・データ拡張・オプティマイザも振る）。**全 10 設定で連結が勝つ**（例: 9M 規模で 54.3 vs 53.2）。ImageNet-1K へのスケールアップでも同傾向。**理論的動機**: 連結は $d_{out}>d_{in}$ を作るのでランクを増やす／中間の次元削減（遷移層）を挟んでもランクは大きく損なわれない → **遷移層を頻繁に挟めばメモリ問題とランク保持を同時に解ける**。**近代化ロードマップ**（(a) DenseNet-201 79.7 → (h) RDNet-T 82.8）: (b) より広く浅く（GR 32→120、精度 −0.2 だが**レイテンシ 38.4 → 8.5ms と 4.5 倍速**）→ (c) ConvNeXt ブロック +0.9 → (d) **ER を GR から切り離す** +0.4 → (e) **遷移層を 3 ブロックごとに挟む +1.5（最大の跳ね）** → (f) パッチ化ステム → (g) 洗練された遷移層 → (h) チャネル再スケーリング。**主要結果**: RDNet-T **82.8**（ConvNeXt-T 82.1 / Swin-T 81.3 / DeiT-III-S 81.4）/ RDNet-B **84.4** / **RDNet-L 384² で 85.8 を 186M で達成**（DeiT-III-L は 304M で同点、レイテンシは 2714 vs 4586ms）/ ADE20K **49.6**（ConvNeXt-B 49.1）/ COCO **47.5 AP^box**（ConvNeXt-T 46.2、しかも 43M vs 48M）/ ゼロショット CLIP **54.1**（ConvNeXt-B 51.2）。**b1 レイテンシが際立つ**: RDNet-T **7.4ms** に対し BiFormer-S 51.6ms（**7 倍差**）。**Appendix の知見**: 転移性で **RDNet-T（24M）が iNaturalist で DeiT-III-L（304M）を上回る**（77.0 vs 75.6）/ **OOD 頑健性で「ImageNet 精度が低くても OOD スコアは高い」**（RDNet-L は ConvNeXt-L に IN で +0.5 だが Avg Shift で +2.3、Sketch で +4.4）/ **Stochastic Depth が +1.2%p**（DenseNet は「接続パターンが似ているので不要」として使っていなかった定石を破った）/ **WGAN の生成器を連結型に置換して FID 27.79 → 25.37**。**CKA 解析**: RDNet は各層で異なる特徴を学ぶが ConvNeXt は中盤の層同士が似る。**限界**: -large までしかスケールしていない（論文が明記）/ 最新モデル（BiFormer / SMT / MogaNet）には**精度で負ける** / ⚠**メモリで ResNet 系に負ける**（RDNet-T 4.1GB vs ConvNeXt-T 2.7GB、「メモリ効率を高める」という要旨の主張は DenseNet 比でのみ成立）/ **速度優位が b1 に偏り b128 では差が縮む** / ⚠**パイロット研究の差が約 1pt で標準偏差（約 2）より小さく、しかも Appendix 0.I.1 で「近代的な訓練レシピが差を縮める」ことを自ら記録**（$\mathcal{E}$ 空間の Sto. Depth / RandErase では差が消える）。
- [[sources/inceptionnext]] — InceptionNeXt「InceptionNeXt: When Inception Meets ConvNeXt」（Weihao Yu, Pan Zhou, Shuicheng Yan, Xinchao Wang / NUS + SMU + Sea AI Lab + Skywork AI, **CVPR 2024**, arXiv:2303.16900）。**[[sources/convnext|ConvNeXt]] への実測ベースの批判**。出発点は 1 つの数字——**ConvNeXt-T は ResNet-50 と同程度の FLOPs（4.5G vs 4.1G）を持つが、A100 上で完全精度で訓練したときのスループットは 575 vs 969 img/s と約 60% しかない**。原因は 7×7 depthwise 畳み込みの**メモリアクセスコスト**で、depthwise は演算量に対して読み書きするデータ量の比が悪く、GPU がメモリ帯域に律速される。**予備実験で 2 つの発見**: (1) カーネルを 7×7 → 3×3 にすると 1.4 倍速いが **−0.6%**（82.1 → 81.5）、(2) ShuffleNet V2 に倣い**一部のチャネルだけ畳み込みに通す**と、比率 1/4 まで −0.2% で済むが 1/16 では −1.4% と崩れる。**提案手法 = Inception depthwise convolution**: チャネルを 4 分割し **`DWConv 3×3` / `DWConv 1×11` / `DWConv 11×1` / 恒等写像**に振り分けて連結する（既定で畳み込みを通るのは全チャネルの **3/8** だけ）。**計算量がカーネルサイズ $k$ の 2 次から 1 次に落ちる**（通常 $k^2C^2$ / depthwise $k^2C$ / Inception depthwise $(2k+9)C/8$）。**MetaNeXt という抽象化**も導入: **MetaFormer** はショートカットを 2 本（トークン混合器用と MLP 用）持つが、**ConvNeXt は 1 本しか持たない**——この 1 本版が MetaNeXt で、ConvNeXt はその Mixer を DWConv 7×7 とした具体化にすぎない。**主要結果**: InceptionNeXt-T 28M/4.2G で **A100 訓練 901 img/s（+57%）・推論 2900（+20%）・82.3（+0.2）**（ConvNeXt-T は 575 / 2413 / 82.1、ResNet-50 は 969 / 3149 / 78.4）→ **ResNet-50 の速度と ConvNeXt-T の精度を同時に取る**。**Atto では訓練 +219%（835 → 2661）**。ADE20K UperNet で **T 47.9 / S 50.0 / B 50.6**（ConvNeXt-T 46.7 / Swin-T 45.8）、Semantic FPN で InceptionNeXt-T **43.1**（PoolFormer-S24 40.3）——**パラメータが少なく FPS が高く mIoU も高い**。**注目すべき負の結果**: **MetaNeXt-Attn（Mixer を自己注意にした版）は top-1 3.9% で収束しない**——ショートカットを 1 本に減らした代償で、「MetaNeXt のトークン混合器は複雑すぎてはならない」。**アブレーション**: 帯状カーネルのどちらか一方を除くと **−0.4（最大の劣化）**／3×3 の除去は −0.3 で済みしかも速くなる／帯状カーネルは **11 で頭打ちで 13 では下がる**（論文は「最適化に起因するかもしれない」と留保）／**BN → LN は精度 +0.1（82.4）だが訓練スループット −20%**（901 → 721）なので速度優先で BN を採用。**限界**: depthwise が $\mathcal{O}(C)$、MLP が $\mathcal{O}(C^2)$ なので**大きいモデルほど効かない**（論文自身が明記）／精度の絶対値では [[entities/rdnet|RDNet-T]] 82.8 に負ける／⚠ スループット測定がバッチサイズ可変（「GPU が収容できるまで減らす」）で 1 条件 1 数字／⚠ BN 採用の副作用（対比学習での情報漏洩等、[[entities/nfnet]] が整理）は未検証。
...
- [[translations/maxvit]] — MaxViT 原論文の全文和訳。Abstract + §1-5 + **Appendix 0.A〜0.C を全訳**（References のみ除外、ユーザー指示で Appendix 含む）。図 1-7 を `<figure>` で埋め込み（**原典 markdown には図 8 のみ画像があったため、図 1-7 は arXiv e-print ソースから生成**。図 8 は GAN サンプル 48 枚のグリッドのため省略）、表 1-14 を markdown 化して収録。Max-SA の Block / Grid 分解の数式、MBConv の定式化、relative attention、**einops 風の MaxViT ブロック擬似コード**、軸注意との比較、5 サイズの詳細アーキテクチャ仕様（表11）、全ハイパーパラメータ（表12）、5 種のアブレーション（表7-10）、完全比較表（表13/14）、GAN の目的関数と美観評価の EMD 損失を含む。

- [[translations/nfnet]] — NFNet 原論文の全文和訳。Abstract + §1-6 + Conclusion + **Appendix A〜E を全訳**（References のみ除外、ユーザー指示で Appendix 含む）。図 1-8 を `<figure>` で埋め込み（**原典 markdown に画像がなかったため arXiv e-print ソースから生成**、図 2 は 2 パネルを連結）、表 1-6 を markdown 化して収録。BN の 4 恩恵の分解、NF 骨格の数式（$\alpha$/$\beta$/SkipInit/Scaled WS）、**AGC の定式化**、F0-F6 の構成（表1）、アーキテクチャ修正とデータ拡張のアブレーション（表2）、モデル比較（表3）、転移での NF vs BN（表4）、大規模事前学習の比較（表5）、BN 版との詳細比較（表6）、全訓練設定（Appendix A）、**BN の実装上の既知バグの列挙**（Appendix B）、ブロック構造の詳細（Appendix C）、AGC 追加アブレーション（Appendix D）、**否定的結果**（Appendix E）を含む。
- [[translations/hrnet]] — HRNet 原論文の全文和訳。Abstract + §I-VIII + **Appendix A〜E を全訳**（References のみ除外、ユーザー指示による §4 逸脱）。図は arXiv e-print の同梱 PDF から生成（定性例の図6/7/8 は省略）。
- [[translations/l2rw]] — L2RW 原論文の全文和訳。Abstract + §1-5 + **Appendix A〜C を全訳**（References のみ除外、ユーザー指示による §4 逸脱）。原典 markdown に画像が 1 枚も埋め込まれていなかったため、図 7 点はすべて arXiv e-print 同梱の PDF から生成。Algorithm 1 は原典で改行が潰れていたため LaTeX ソースからステップ順を復元。
- [[translations/medgemma]] — MedGemma 原論文の全文和訳。Abstract + §1-10 + **Appendix A・C・D・F を全訳**（ユーザー指示による §4 逸脱）。**Appendix B（Evaluation Prompts）は英語プロンプト文字列の列挙のため要旨のみ**、Appendix E は代表例の構造のみ。図 8 点は arXiv e-print 同梱の PDF から生成。
- [[translations/medgemma-1-5]] — MedGemma 1.5 原論文の全文和訳。Abstract + §1-6 + **Appendix A（CT-RATE 評価）を全訳**。**Appendix B（Prompts）は要旨のみ**。図 3 点は arXiv e-print 同梱の PDF から生成。
- [[translations/biomedclip]] — BiomedCLIP 原論文の全文和訳。Abstract + Introduction + Results + Discussion + Methods + **Supplementary Note（アーキテクチャの詳細とアブレーション研究、補足表 1-10）を全訳**（ユーザー指示による §4 逸脱）。References のみ除外。原典 markdown には**図3 が欠落**していたため、図 5 点はすべて arXiv e-print 同梱の PDF から生成。
- [[translations/elr]] — ELR 原論文の全文和訳。Abstract + §1-8 + **Appendix A〜I を全訳**（ユーザー指示による §4 逸脱）。**Appendix A（線形モデルにおける早期学習と暗記の理論解析）は命題 3・定理 4・命題 5・定理 6・補題 7-10 とその証明**からなり、数式は LaTeX のまま保持して地の文を訳している。References のみ除外。図 7 点は arXiv e-print 同梱の PNG から複数パネルを組み合わせて生成。
- [[translations/rdnet]] — RDNet 原論文の全文和訳。Abstract + §1-6 + **Appendix 0.A〜0.I を全訳**（ユーザー指示による §4 逸脱）。References のみ除外。原典 markdown には画像が 4 枚しか埋め込まれておらず**看板図であるアーキテクチャ図（図1）が欠落**していたため、図 10 点はすべて arXiv e-print 同梱の PDF から複数パネルを組み合わせて生成。ar5iv 変換で §5.2〜5.3 の本文が二重出力されていたため重複を除いている。
- [[translations/inceptionnext]] — InceptionNeXt 原論文の全文和訳。Abstract + §1-5 + **Appendix A・B も全訳**（Appendix がハイパーパラメータ表と Grad-CAM の定性結果のみで分量が小さいため）。References のみ除外。**原典 markdown（ar5iv 由来）には画像が 1 枚（Grad-CAM の 1 コマ）しか埋め込まれておらず、看板図である図1（精度 vs スループット）・図2（ブロックの図解）・図3（FLOPs の比較）がすべて欠落**していたため、図 4 点は arXiv e-print 同梱の PDF から生成（図4 の Grad-CAM は 5 行 ×4 列を手動で組み合わせ）。Algorithm 1 の PyTorch コードと表 1〜10 の巨大 HTML テーブルを markdown に変換。
...
- [[concepts/example-reweighting]] — 事例の重み付け。訓練データのバイアス（クラス不均衡・ラベルノイズ・データセットポイズニング）に、データを捨てずに対処する枠組み。核心は「**何を根拠に重みを決めるか**」で、**hard 優先（AdaBoost / hard negative mining / Focal Loss）と easy 優先（self-paced learning / Reed bootstrapping）が正面から矛盾する**という構図を整理する。第三の道として [[sources/l2rw]] の「検証損失の勾配方向との一致度」を解説。**乱数の重みというベースラインを必ず置くべき**という教訓（L2RW の Random が既存手法を全部倒した件）、[[entities/flexmatch]] の CPL が SVHN で失敗した件との**粒度の対比**（クラス単位 vs 事例単位）、[[entities/dfn]] との**骨格の同型性**（小さな高品質データで大量の粗いデータの使い方を決める / 違いは粒度とタイミング）も収録。一次情報が L2RW 1 件のみのため、それを超える記述には ⚠ を付けている。
- [[concepts/medical-foundation-model]] — 医療基盤モデル。**データを外に出せない / モダリティが極端に多様 / 正解に専門医が要る / 失敗のコストが非対称**という 4 制約が設計を規定する。**3 類型**（単一モダリティ特化 SSL = [[entities/eva-x]] **6M で SOTA** / 合成データ SSL = [[entities/i-synmed]] / 汎用基盤モデルのドメイン適応 = [[entities/medgemma]]・[[entities/mini-internvl]]）を整理し、**「汎用性を壊さずに専門性を足す」手口**（元データを残して 2%・10% で混ぜる / 不要な段階で足さない / SFT でなく RL / 部品を凍結 / モダリティ別の補助教師）を [[concepts/knowledge-distillation]] や [[concepts/parameter-efficient-fine-tuning]] とは**別系統の「忘却を防ぐ」実務**として収録。**汎用 vs 特化の非対称**（テキストの医学知識は汎用が勝ち、視覚は特化が勝つ／**ドメイン特化の利得はそのモダリティの Web 上での希少性と相関する**）、**高次元画像を前処理と長コンテキストで解く**という設計、**評価の難しさ**（ベンチマークの飽和 / 「正しい患者管理に至るか」を採点軸にするルーブリック / 幻覚は代表図にすら写る）も扱う。
- [[concepts/learning-with-noisy-labels]] — ノイズラベル学習。中心にあるのは **「早期学習 → 暗記」という訓練動力学**（深層ネットワークはまずクリーンなラベルに適合し、**誤ラベル事例に対してすら正しいクラスを予測できる**時期を経てから、誤ったラベルを覚え込む）。[[sources/elr]] がこれを**線形モデルで証明**し、暗記の原因が**深層性ではなく高次元性**であることを示した。対処の **5 類型**（① ロバスト損失 / ② 損失補正 / ③ サンプル選択・重み付け / ④ ラベル修正 / ⑤ 正則化）を整理し、③ の**構造的な自己矛盾**（「損失が小さいからクリーン」→「選ばれたものは勾配も小さい」→「学習が進まない」）、⑤ の設計の核心（**「目標に近づける」と「目標が指す方向へ押す」は違う** / 自己参照のループは正則化なしには壊れる）、**「何を正しさの基準にするか」の 3 分類**（外部のクリーンなデータ = [[sources/l2rw]] / モデル自身の過去 = [[sources/elr]] / もう 1 つのモデル = Co-teaching 系）を収録。[[concepts/semi-supervised-learning]] との**方法論的な合流**（ELR+ の 4 要素はすべて半教師あり由来、高ノイズほど寄与が支配的）と、**評価の落とし穴**（[[sources/l2rw]] の「人工ノイズでは強い正則化をかけるだけでうまくいく」という警告は正則化型の手法に特に強く当たる）も扱う。
- [[concepts/skip-connection]] — スキップ接続（ショートカット）。**「勾配を運ぶ」「特徴を運ぶ」「解像度を運ぶ」の 3 目的が同じ名前で呼ばれている**ことを整理し、**合流のさせ方（加算 / 連結 / 融合）**で設計が分かれることを扱う。**加算 vs 連結**の比較表（チャネル数が変わるか / 特徴の再利用 / メモリと幅のスケーリング）、[[sources/rdnet]] の**ランクの議論**（連結は $d_{out}>d_{in}$ を作る）と **15,000 個超のランダムネットワークによる検証**（RegNet 由来の「設計空間を population として評価する」手法）、**連結の代償とその解き方**（遷移層を頻繁に挟む）、**CKA が示す表現の分化**、そして「**skip があるから他の要素は要らない**」という定石を疑う議論（DenseNet の Stochastic Depth 不使用 → +1.2%p / [[entities/nfnet]] の正規化排除 / [[entities/hiera]] の特殊モジュール排除）を収録。[[entities/hrnet]] の多解像度融合を「解像度を運ぶ skip の極端な形」として位置づける。**第 3 の軸として「ショートカットの本数」**も扱う——[[sources/inceptionnext]] の **MetaNeXt**（MetaFormer の 2 本を 1 本に統合）は、**1 本のブロックに自己注意を置くと収束しない**（top-1 3.9%）ことを示し、「skip を減らすと他の部品が置けなくなる」という逆向きの依存を実証した。
...
- [[entities/maxvit]] — MaxViT（Google Research + UT Austin, **ECCV 2022**）。**block attention（局所窓内）+ grid attention（等間隔に間引いた大域）+ MBConv を 1 ブロックに統合したハイブリッド**。名前は **M**ulti-**ax**is **Vi**sion **T**ransformer の略。$P=G=7$ で局所と大域の計算量が釣り合い**双方とも線形**、**第 1 層から大域視野**を持つ。**Swin の attention と同パラメータ・同 FLOPs の drop-in 置換**で cyclic shift もマスクも不要。MBConv の depthwise conv が **CPE** として働き位置埋め込み不要。**T (31M) / S (69M) / B (120M) / L (212M) / XL (475M)**、段階配分は {2,2,5,2}（T/S）・{2,6,14,2}（B/L/XL）で **Swin の {2,2,18,2} より大モデルで優位**。IN-1K 224² 83.62/84.45/84.95/85.17、**512² で MaxViT-L 86.70（当時の通常訓練 SOTA）**、**IN-21K MaxViT-B 88.38（CoAtNet-4 をパラメータ 43% で超え）/ XL 88.70**、JFT **89.53**、COCO 896² で **53.4 AP**、AVA 美観 PLCC **0.745**、GAN で FID **30.77**/IS **22.58** を 18.6M で。**弱点**: スループットが **Swin/ConvNeXt の半分以下**（349.6 vs 755.2/774.7）、IN-1K のみではパラメータ効率も劣る、最高値は非公開 JFT 依存。Apache 2.0、`github.com/google-research/maxvit`、`timm` 収録。**[[entities/hiera]] とは設計思想が対極**（削ぐ vs 統合する）。
- [[entities/nfnet]] — NFNet / NF-ResNet（DeepMind, **ICML 2021**）。**BatchNorm を完全に排除した純粋 CNN**。**AGC**（勾配ノルム/重みノルム比を**ユニット単位**でクリップ、$\lambda=0.01$、最終線形層は除外）+ **Scaled Weight Standardization**（平均シフト除去）+ **分散の解析的予測**（$\beta_i$）+ **SkipInit** で BN の役割を代替。SE-ResNeXt-D ベースで**グループ幅 128 固定 / 深さ $[1,2,6,3]\times N$ / 幅 $[256,512,1536,1536]$**、**実機の訓練レイテンシを最適化対象**にした設計。**F0 (71.5M, 83.6) / F1 (132.6M, 84.7) / F2 (193.8M, 85.1) / F3 (254.9M, 85.7) / F4 (316.1M, 85.9) / F5 (377.2M, 86.0→SAM 86.3) / F6 (438.4M, SAM 86.5)** + **F4+**（幅広版 527M、JFT 事前学習で **89.2%**）。**NFNet-F1 が EffNet-B7 と同精度を 8.7× 速い訓練で**、**同一アーキの BN 版より速く（20-40%）精度も高い**、**転移では NF が BN を全サイズで約 1pt 上回る**。**弱点**: パラメータ効率が悪い（F1 132.6M vs EffNet-B7 66.0M で同精度）、最高値は非公開 JFT 依存、調整項目が多い、**attention 未試行**。Apache 2.0（JAX/Haiku）、`timm` に `nfnet_f0`〜`f6` / `nf_resnet50` 等。**[[entities/convnext]] は翌年 BN → LN という別解を採用**し、BN 排除路線は主流にならなかった。[[questions/cnn-beyond-convnext-v2]] で引用した **89.2%** の原典。
- [[entities/hrnet]] — HRNet / HRNetV1 / V2 / V2p（Microsoft Research Asia, **TPAMI 2021**）。**解像度を落とさない CNN バックボーン**。幅 $C$ でサイズが決まり（W18 / W32 / W40 / W48 / W96）、4 解像度の幅は $C, 2C, 4C, 8C$。**位置に敏感なタスク（姿勢推定・セグメンテーション・検出・顔ランドマーク）の標準**として定着し、[[sources/segment-anything|SAM]] の対話的セグメンテーション比較（RITM の HRNet32）や [[sources/dino-x|DINO-X]] の姿勢推定表に**2023〜2025 年でも比較対象として登場する**。[[entities/swin-transformer]] が「CNN の階層性を Transformer に輸入」したのに対し、HRNet は「CNN の階層性を並列化」した — 同じ密 prediction 問題への両側からの回答。
- [[entities/medgemma]] — MedGemma（Google Research + DeepMind, 2025）。**[[entities/gemma-3]] のアーキテクチャそのまま**で医療特化した視覚–言語基盤モデル。**4B マルチモーダル / 27B テキスト / 27B マルチモーダル**の 3 変種。**4B PT と 4B IT は用途が違う**（CXR レポート生成は RadGraph F1 がスタイルに敏感なため **PT の方が良い**）。訓練データの中核（Internal histopathology 3,260 万パッチ等）は**非公開**。HAI-DEF として <https://goo.gle/medgemma> で公開。
- [[entities/medgemma-1-5]] — MedGemma 1.5（同, 2026）。**4B のみ**（27B は 1.0 のまま）。MedSigLIP を**凍結**し言語デコーダのみ更新。3D は**軸位断 85 スライス = 21,760 トークン**、WSI は**126 パッチ = 32,256 トークン**で**空間順序を保存**。**バウンディングボックスを JSON で出力**できるようになった。**MMLU Pro が Gemma 3 4B 比 -9.8pt** という 4B クラスの明確なトレードオフを論文自ら認めている。
- [[entities/biomedclip]] — BiomedCLIP（Microsoft Research + UW, 2023）。**ViT-B/16-224 + PubMedBERT/256**、[[entities/pmc-15m]] で事前学習した生物医学の CLIP。**[[entities/medsiglip]] とほぼ全項目で正反対の設計**（ゼロから vs SigLIP 出発 / 医療 100% vs 医療 2% / InfoNCE vs sigmoid 損失 / 完全再現可能 vs 重みのみ公開 / 汎用性を捨てる vs 保持する）。**LLaVA-Med の視覚エンコーダ**として使われた。HF: `microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224`
- [[entities/pmc-15m]] — PMC-15M（Microsoft Research, 2023）。**PubMed Central Open Access Subset の 440 万件の全文論文から抽出した 15,282,336 の図–キャプション対**。`entity_kind: dataset`。MIMIC-CXR（37.7 万）の**約 40 倍**、**30 の画像型**を網羅。**データセット自体でなく「PMC-OA から再現するスクリプト」の形で提供される**（元論文のライセンスが個別に異なるため）。⚠ **最頻の画像型は統計図表・グラフ・表など汎用の科学図解**で、放射線・病理はその下。複合図をパネル分割し引用箇所を加えた **PMC-Fine-Grained-46M** も作られたが、本論文での用途は画像型の分布の集計のみ。[[entities/wit-400m]] / [[entities/lvd-142m]] が非公開なのと対照的に**完全公開**
- [[entities/rdnet]] — RDNet / Revitalized DenseNet（NAVER Cloud AI + NAVER AI Lab, **ECCV 2024**）。**DenseNet を近代化した CNN バックボーン**。T/S/B/L の 4 スケール（24M/50M/87M/186M）。**特徴混合器の出力が GR 次元**（ConvNeXt のブロックが入力と同次元を返して加算するのに対し、RDNet は GR 次元だけ返して**連結**する）。**GR はステージごとに異なる**（例: T は 64, 104, 128, 224）——一様な GR は精度と効率の両方を損なう。**遷移層が次元削減とダウンサンプリングを兼ね、3 ブロックごとに挟まれる**。`timm` に `rdnet_tiny`〜`rdnet_large` として収録。<https://github.com/naver-ai/rdnet>
- [[entities/medsiglip]] — MedSigLIP（同, 2025）。**[[entities/siglip]]-400M を医療画像–テキスト対 3,300 万で微調整**した 4 億パラメータの医療画像エンコーダ。**元の WebLI を残し医療を 2% で混合**。**448² と 896² は同一重み**（位置埋め込みのダウンサンプルのみ）。ゼロショット CXR で **CXR Foundation（ELIXR）を平均 2.0% 上回る**——**解像度 448² 対 1280² の不利、かつ 4 領域兼任で**。ただし **linear probe では最小の 64 枚で専用エンコーダに負け、512 枚以降で逆転**する。[[entities/internvit-300m]] と同じ「MLLM の視覚エンコーダを独立公開する」設計。
...
- [[entities/inceptionnext]] — InceptionNeXt（NUS + SMU + Sea AI Lab + Skywork AI, **CVPR 2024**）。**ConvNeXt の 7×7 depthwise 畳み込みを 4 分岐に分解した CNN バックボーン**。`DWConv 3×3`（$C/8$）+ `DWConv 1×11`（$C/8$）+ `DWConv 11×1`（$C/8$）+ **恒等写像**（$5C/8$）を連結する **Inception depthwise convolution** が中核で、**畳み込みを通るのは全チャネルの 3/8 だけ**。**計算量がカーネルサイズ $k$ の 1 次**（$(2k+9)C/8$）。A / T / S / B の 4 スケール。ブロック構造は ConvNeXt をほぼ踏襲するが、**速度優先で LayerNorm ではなく BatchNorm**、**第 4 ステージだけ MLP 比 3**（他は 4）。InceptionNeXt-T 28M/4.2G で **A100 訓練 901 img/s・82.3**（ConvNeXt-T は 575・82.1）、**Atto は訓練 +219%**。ADE20K UperNet で T 47.9 / S 50.0 / B 50.6。**MetaNeXt**（MetaFormer の 2 本のショートカットを 1 本に統合した抽象ブロック）を定義し、その Mixer を自己注意にすると**収束しない**（3.9%）ことも報告——[[concepts/skip-connection]] に「ショートカットの本数」という軸を与えた。**効くのは軽量モデルに限る**（depthwise $\mathcal{O}(C)$ vs MLP $\mathcal{O}(C^2)$）。`timm` に `inception_next_atto`〜`_base` として収録。<https://github.com/sail-sg/inceptionnext>
...
- [[questions/convnext-v1-vs-v2-architecture]] — **ConvNeXt V1 と V2 にアーキテクチャの違いはあるか？「V2 = V1 + FCMAE」なのか**。結論: **違いはあるが極小**（GRN 層を 1 つ追加 + LayerScale 削除、stem・stage 構成・カーネルは同一）。ただし**「単に V1 に FCMAE をかけたもの」は論文が明示的に否定した仮説**で、V1+FCMAE は 83.7 と教師あり 300ep の 83.8 にすら届かない。原因は**特徴崩壊**（次元拡大 MLP でチャネル間の活性化が冗長化）で、GRN がこれを防ぐ。**FCMAE だけでも GRN だけでも効かず、両方揃って初めて効く**（V2-B +0.8 / V2-L +1.3、スケールするほど差が開く）という **co-design** の主張。**GRN は事前学習と FT の両方に必要**（片方だけだと 78.8 / 80.6 とベースライン以下）でモデルの恒久的な一部。副次的にラインナップも変化（**V2 に Small と XLarge は存在せず**、超軽量帯 Atto〜Nano と Huge を新設。T/B/L の構成は V1 と同一）。[[entities/hiera]] との「同年に Transformer 側と ConvNet 側から同じ co-design の結論に到達」という対比、および「V1 が MIM に向かないのは CNN の本質的限界ではない」という精緻化を含む。
- [[questions/cnn-beyond-convnext-v2]] — **CNN で ConvNeXt V2（88.9%）を超えるモデルはあるか**。結論: **ある**が、**同じデータ条件で超えたものはない**。wiki 内の反例は **NFNet-F4+ 89.20%**（[[translations/maxvit]] 表3、ただし **JFT-300M = 非公開・IN-22K の約 21 倍**）。wiki 外の本命は **InternImage-H 89.6%**（DCNv3、公開データだが約 30 倍の 4 億枚規模、**未取り込みのため確度低**）。最大の論点は「**それは本当に CNN か**」——**DCNv3 は入力に応じてサンプリング位置を学習する**ため attention の内容依存性を借りており、「純粋 CNN が超えた」ではなく「**attention の性質を借りた畳み込みが超えた**」と読むのが正確。**静的カーネルに限れば ConvNeXt V2 は今も上位**。さらに **ImageNet 分類 SOTA 競争自体が 2023 年以降主戦場から外れた**（凍結特徴量 / 言語整合性 / MLLM 視覚エンコーダへ移行、CNN は [[entities/dinov3]] の蒸留先として「効率が要る領域」に定着）。**比較を読むときのチェックリスト 5 項目**（データ規模と公開性 / params と FLOPs / 訓練レシピ / 解像度 / アーキテクチャの定義）を含む。
- [[questions/vit-architecture-evolution]] — **ViT (2020) から DINOv3 / SigLIP 2 までのアーキテクチャ進化 — 8 軸の変化**。ユーザの「アーキテクチャは ViT からあまり変化していないのでは？」への回答。**結論: 半分正しい**（基本骨格は不変だが、構成要素のほぼすべてが入れ替わっている）。**不変な部分**: Patchify + Transformer Encoder × L + MSA + MLP + 残差 + Pre-norm の核心骨格。**8 つの変化軸**: ① **位置エンコーディング**（学習可能 1D → RoPE 系、最大の変化）、② **Norm**（LayerNorm → RMSNorm + QK-norm + LayerScale）、③ **MLP 活性化**（GELU → SwiGLU）、④ **Register tokens の追加**（[CLS] のみから脱却、DINOv2/v3）、⑤ **Attention 効率化**（完全 SA → Window/階層型、Qwen2.5-VL の 4/32 層）、⑥ **出力読み出し**（最終 [CLS] → 中間層活用、PE/DeepStack/InternViT V2.5 発見）、⑦ **解像度処理**（固定 → 任意、NaFlex/Naive Dynamic Resolution/Pan & Scan）、⑧ **トークン圧縮機構の追加**（Pixel Shuffle/Merger/Average Pooling/ViR）。**DINOv3 vs SigLIP 2 の対比**で「アーキテクチャ革新型 vs 学習革新型」の路線分化を可視化。**メタ観察**: 「**NLP からの逆輸入**」が変化の通底テーマ（RoPE / SwiGLU / RMSNorm / YaRN / FlashAttention 等）→ 「ViT 時代の Vision 独自進化」から「Vision と Language が同じアーキテクチャ要素を共有」への移行。ユーザが指摘する「**学習・データの工夫が大幅に改善**」も妥当で、**学習・データ・アーキテクチャの 3 軸並行進化** が 2020-2025 の 5 年間の総括として最適。
- [[questions/vlm-vision-encoder-continued-pretraining]] — **VLM の vision encoder を継続事前学習する 6 つの目的**。wiki の複数 MLLM 事例（Qwen-VL 系 / InternVL 系 / Gemma 3 / DeepSeek-OCR）を横断整理。**6 つの目的軸**: (1) 解像度・アスペクト比の柔軟化（固定 224²/384² → 任意解像度）、(2) 位置エンコーディングの変更（学習可能 1D → 2D-RoPE / M-RoPE / Interleaved MRoPE）、(3) OCR・文書・図表理解能力の強化（自然画像 → 文書・表・チャート、多言語対応）、(4) 計算効率の改善（Pixel Shuffle、Window Attention、ViR 等）、(5) LLM と整合する特徴空間への整形（[[concepts/alignment-tuning|alignment tuning]]、DeepStack、Native Multimodal Pre-Training）、(6) 追加モダリティ・タスクへの適応（動画・3D・グラウンディング）。**例外: 凍結戦略**（[[entities/gemma-3|Gemma 3]] が SigLIP-400M を凍結 + Pan & Scan で部分対応）の理由とトレードオフ。**11 モデルの継続事前学習戦略一覧表**で具体的に比較（Qwen-VL〜Qwen3-VL / InternVL〜InternVL 3.5 / Gemma 3 / DeepSeek-OCR）。**3 つの戦略分類**: 積極的継続学習（Qwen 系 / InternVL 1.5）vs 凍結（Gemma 3 / InternVL 3.5 の InternViT-V2.5）vs ハイブリッド（DeepSeek-OCR の SAM+CLIP 直列）。本質的には**「汎用視覚モデル → MLLM 用カスタマイズされた視覚モジュール」への変換**で、訓練コスト vs カスタマイズ度のトレードオフが選択を決める。
- [[questions/sigmoid-vs-softmax-loss]] — **SigLIP の sigmoid 損失はなぜ memory 効率と分散学習で有利なのか**。CLIP の softmax 損失と並べた段階的解説。**鍵は「セル間の独立性」**: softmax は各行に **グローバル正規化** が必要で **$N \times N$ 行列全体を materialize** しなければ計算できないが、sigmoid は **各 $(i,j)$ セルが独立な二値分類** で **ブロック単位で計算・破棄** できる。**具体的な数値**: バッチ 32k で softmax は 4 GB+（行全体）、sigmoid は 4 MB（1024² ブロック）→ **1000 倍の削減**。**分散学習**: softmax は全 GPU 埋め込みの **all-gather** 必要、sigmoid は **テキスト・チャンクの順送り** だけ（ring all-reduce 風）。**直感的例え**: softmax = 競争（試験の偏差値、全員の点数が必要）vs sigmoid = 個別判定（合否判定、他は不要）。**学習可能バイアス $b_0 = -10$ で正例/負例不均衡を補正**する設計、**バッチサイズ 32k で性能飽和**の発見（CLIP の「大バッチ無限スケール」の常識を覆す）も含む。**CV/MLLM での影響**: SO-400M で 5B EVA-CLIP を凌駕（83.2% IN-0）、Qwen3-VL の SigLIP-2 継続学習、Gemma 3 の SigLIP 400M 共有など、現代 MLLM の vision encoder の事実上の標準。
- [[questions/rope-extrapolation-mechanism]] — **RoPE は学習範囲を超えた相対位置にどこまで適応できるか**。「形式的可能性（数式は連続値で計算可能）」と「実用的可能性（学習器の経験範囲）」の **2 段階を区別**する分析。**鍵となる洞察**: RoPE の各次元 $i$ は異なる波長 $\lambda_i = 2\pi b^{2i/d}$ を持ち、**低周波次元（高い $i$、長い波長）は訓練中に 1 回転すら経験していない**ため、訓練範囲を超えた回転角は完全に未学習。具体例として $d=64, L=1000$ で $i=31$ の波長は 55,000 トークン → 訓練中に **1.8% しか回転を経験しない**。相対位置 1999 では同次元で 3.6% の回転になり「**訓練分布外**」となる。**Attention の内積は全次元の和なので、低周波次元が異常値を返すと内積全体が壊れる**。これが [[sources/roformer|RoFormer]] §4.5.5 で著者 Su Jianlin 自身が認めた「長文脈外挿の挙動の理論的説明欠如」の正体。解決法は **PI → NTK-aware → NTK-by-parts → Dynamic NTK → [[sources/yarn|YaRN]]** の 5 世代系譜で、特に **NTK-by-parts は波長ごとに戦略を変える**（$\lambda < L$ は補間せず、$\lambda \geq L$ は完全補間）。**ピアノの鍵盤の高音 vs 低音**の例えで説明。CV/MLLM での実用例（Qwen2-VL 16K→80K、**Qwen3-VL 256K ネイティブ + 1M YaRN Needle 99.5%**、DINOv3 axial RoPE + box jittering）を含む。
...
| Max-SA | Multi-Axis Self-Attention（block attention + grid attention の組） | [[sources/maxvit]] / [[entities/maxvit]] |
| block attention | P×P の重複しない窓の内部で attention（Swin の W-MSA 相当の局所注意） | [[sources/maxvit]] |
| grid attention | G×G の一様グリッドの格子点どうしで attention（dilated な大域注意） | [[sources/maxvit]] |
| MBConv | Mobile Inverted Bottleneck Conv（MobileNetV2 由来、Conv1×1 → DWConv3×3 → SE → Conv1×1） | [[sources/maxvit]] / [[concepts/convolutional-neural-network]] |
| CPE | Conditional Position Encoding。depthwise conv が位置情報を担い明示的な位置埋め込みを不要にする | [[sources/maxvit]] |
| axial attention | 列方向 → 行方向に順に attention する手法（O(N√N)）。Max-SA とは別物 | [[sources/maxvit]] |
| CoAtNet | Google の先行ハイブリッドモデル。MaxViT の最大の比較対象（本 wiki 未取り込み） | [[sources/maxvit]] |
| AVA | 画像美観評価のベンチマーク（255K 画像、平均 200 人が 1-10 で評定） | [[sources/maxvit]] |
| PLCC / SRCC | Pearson の線形相関係数 / Spearman の順位相関係数 | [[sources/maxvit]] |
| FID / IS | Fréchet Inception Distance（低いほど良い）/ Inception Score（高いほど良い） | [[sources/maxvit]] |
| JFT-300M | Google 内部の約 3 億枚の弱ラベル画像データセット（非公開） | [[sources/maxvit]] |
...
| BN | Batch Normalization | [[concepts/convolutional-neural-network]]（BN vs LN 節） / [[sources/nfnet]] |
| NFNet / NF-ResNet | Normalizer-Free Network / ResNet（正規化層を持たない CNN） | [[entities/nfnet]] / [[sources/nfnet]] |
| AGC | Adaptive Gradient Clipping（勾配ノルム/重みノルム比でユニット単位にクリップ） | [[sources/nfnet]] |
| Scaled Weight Standardization | 畳み込みの重みを正規化して平均シフトを消す再パラメータ化 | [[sources/nfnet]] |
| mean shift（平均シフト） | ReLU/GELU が反対称でないため深さに比例して事例間の活性化が相関する現象 | [[sources/nfnet]] |
| SkipInit | 各残差分岐末尾のゼロ初期化スカラー。ブロックを恒等写像から始める | [[sources/nfnet]] |
| SAM | Sharpness-Aware Minimization（平坦な最小値を探す最適化、既定で訓練時間 2 倍） | [[sources/nfnet]] |
| HRNet | High-Resolution Network（過程全体で高解像度を維持する CNN） | [[entities/hrnet]] / [[sources/hrnet]] |
| HRNetV1 / V2 / V2p | HRNet の 3 つの表現ヘッド（高解像度のみ / 4 解像度連結 / V2 + 特徴ピラミッド） | [[entities/hrnet]] |
| W32 / W48（HRNet） | 高解像度ストリームの幅 C。他の解像度は 2C, 4C, 8C | [[entities/hrnet]] |
| position-sensitive task | 出力が空間位置と結びつくタスク（姿勢推定・セグメンテーション・検出） | [[sources/hrnet]] |
| dilated conv / atrous conv | 拡張畳み込み。カーネル要素の間隔を空けて解像度を保ったまま受容野を広げる | [[sources/hrnet]] |
| 多解像度融合（multi-resolution fusion） | 並列する全解像度が全解像度から情報を受け取る双方向の交換モジュール | [[sources/hrnet]] |
| OCR（セグメンテーション） | Object-Contextual Representation（同一クラス画素の表現を集約して文脈を作る） | [[sources/hrnet]] |
| ASPP | Atrous Spatial Pyramid Pooling（DeepLab の多スケール文脈モジュール） | [[sources/hrnet]] |
| NME | Normalized Mean Error（顔ランドマーク検出の指標） | [[sources/hrnet]] |
| OHKM | Online Hard Keypoint Mining（難しいキーポイントに損失を集中） | [[sources/hrnet]] |
| syncBN | 複数 GPU にまたがって統計を取る Batch Normalization | [[sources/hrnet]] |
| MedGemma | Gemma 3 ベースの医療視覚–言語基盤モデル | [[entities/medgemma]] / [[sources/medgemma]] |
| MedGemma 1.5 | 3D・WSI・局在化・経時解析を足した後継（4B のみ） | [[entities/medgemma-1-5]] / [[sources/medgemma-1-5]] |
| MedSigLIP | SigLIP-400M を医療微調整した画像エンコーダ（448²） | [[entities/medsiglip]] |
| HAI-DEF | Health AI Developer Foundations（Google の医療 AI 基盤モデル群） | [[entities/medgemma]] |
| MedQA / MedMCQA | USMLE 形式 / インド医学入学試験の医療 QA | [[sources/medgemma]] |
| PubMedQA / MedExpQA / AfriMed-QA | PubMed 抄録 / スペイン研修医試験 / 汎アフリカの医療 QA | [[sources/medgemma]] |
| MedXpertQA | 難度の高い分布外の医療ベンチマーク（テキスト / MM） | [[sources/medgemma]] |
| AgentClinic | 模擬患者エージェントと対話して診断に至るエージェント評価 | [[sources/medgemma]] |
| CXR | Chest X-Ray（胸部 X 線） | [[concepts/medical-foundation-model]] |
| MIMIC-CXR / CheXpert / CXR14 | MIT / Stanford / NIH の胸部 X 線データセット | [[sources/medgemma]] |
| EyePACS | 糖尿病網膜症グレーディングの眼底画像データセット | [[sources/medgemma]] |
| SLAKE / VQA-RAD | 医療 VQA の標準ベンチマーク | [[sources/medgemma]] |
| PathMCQA / DermMCQA | 病理・皮膚科の多肢選択評価（内部データ） | [[sources/medgemma]] |
| RadGraph F1 | 生成レポートの所見グラフを正解と比較する指標（**スタイルに敏感**） | [[sources/medgemma]] |
| ELIXR / CXR Foundation | HAI-DEF の胸部 X 線埋め込みモデル（MedSigLIP の比較対象） | [[entities/medsiglip]] |
| linear probe（データ効率的分類） | エンコーダを凍結し線形分類器のみ訓練して表現の質を測る評価 | [[entities/medsiglip]] / [[concepts/knn-evaluation-protocol]] |
| test-time scaling (TTS) | 推論時に計算量を増やして精度を上げる手法 | [[sources/medgemma]] |
| IT teacher | instruction-tuned teacher（蒸留元の指示調整済み大規模モデル） | [[sources/medgemma]] |
| WSI | Whole Slide Image（病理標本をデジタル化した数ギガピクセル画像） | [[sources/medgemma-1-5]] |
| HU（Hounsfield Unit） | CT の画素値の物理単位（水 = 0、空気 = -1000、骨 > +1000） | [[sources/medgemma-1-5]] |
| window-width / window-level | CT のウィンドウ処理の幅と中心値（読影条件） | [[sources/medgemma]] |
| 軸位断 / 矢状断 / 冠状断 | axial / sagittal / coronal（体を切る断面の向き） | [[sources/medgemma-1-5]] |
| T1w / T2w / GRE / SWI | MRI の撮像シーケンスの種類 | [[sources/medgemma-1-5]] |
| CT-RATE | 公開の胸部 CT データセット（18 状態のラベル） | [[sources/medgemma-1-5]] |
| Chest ImaGenome | 胸部 X 線に解剖学的バウンディングボックスを付与したデータセット | [[sources/medgemma-1-5]] |
| MS-CXR-T | 経時的 CXR の推移（改善/安定/悪化）を判定するベンチマーク | [[sources/medgemma-1-5]] |
| EHRQA / EHRNoteQA | 合成 FHIR 記録の QA / MIMIC-IV 退院サマリの臨床推論ベンチマーク | [[sources/medgemma]] / [[sources/medgemma-1-5]] |
| LOINC / FHIR | 検査項目の標準コード体系 / 医療情報交換の標準規格 | [[sources/medgemma-1-5]] |
| Synthea | 合成患者記録の生成器（EHRQA の基） | [[sources/medgemma-1-5]] |
| ROUGE-L | 最長共通部分列に基づく生成テキストの指標（**トークン重複のみ**） | [[sources/medgemma-1-5]] |
| ISIC | 皮膚病変のダーモスコピー画像の公開データセット | [[sources/medgemma-1-5]] |
| PolyPath / CoCa-CXR / BioViL-T | WSI レポート生成 / CXR 局在化 / CXR 経時解析の外部 SOTA | [[sources/medgemma-1-5]] |
| MAIRA-1 / MAIRA-2 / MedVersa | CXR レポート生成の比較対象 | [[sources/medgemma]] |
| Med-Gemini / Med-PaLM M | Google の先行医療モデル（本 wiki 未取り込み） | [[sources/medgemma]] |
| SIIM-ACR / CRC100k | 気胸分類 / 大腸組織病理分類の外部ベンチマーク | [[sources/medgemma]] |
| BiomedCLIP | PMC-15M で訓練した生物医学の CLIP | [[entities/biomedclip]] / [[sources/biomedclip]] |
| PMC-15M | PubMed Central の図–キャプション 1,528 万対 | [[entities/pmc-15m]] |
| PMC / PMC-OA | PubMed Central / その Open Access Subset | [[entities/pmc-15m]] |
| PMC-Fine-Grained-46M | PMC-15M の複合図をパネル分割し引用箇所を加えた拡張版 | [[entities/pmc-15m]] |
| PubMedBERT | 生物医学の文献で事前学習された BERT | [[entities/biomedclip]] |
| citance | 論文本文中で図を参照している文 | [[entities/pmc-15m]] |
| composite figure（複合図） | 1 枚に複数パネル（A, B, C…）を含む図 | [[entities/pmc-15m]] |
| WordPiece / BPE | サブワードのトークン化（ユニグラム尤度 / 頻度ベースの貪欲な結合） | [[sources/biomedclip]] |
| InfoNCE | CLIP の対比損失（バッチ内の正例対を近づけ非対を遠ざける） | [[concepts/contrastive-learning]] / [[sources/biomedclip]] |
| patch dropout | 訓練時に画像パッチの一部を落として効率を上げる手法 | [[sources/biomedclip]] |
| sharding contrastive loss | InfoNCE と同一の勾配で GPU ごとに必要な類似度だけ計算する実装 | [[sources/biomedclip]] |
| Recall@k (R@k) | 正解が上位 k 件に入っている割合 | [[sources/biomedclip]] |
| ELEVATER | 汎用領域の視覚–言語モデルの評価ツールキット | [[sources/biomedclip]] |
| PCam / PatchCamelyon | リンパ節切片 96×96 パッチ 32.8 万枚、転移組織の有無 | [[sources/biomedclip]] |
| LC25000 | 肺・結腸の組織病理画像 2.5 万枚、5 クラス | [[sources/biomedclip]] |
| TCGA-TIL | TCGA 肺腺癌の H&E パッチ 2,480 枚（**5.9% のみ陽性**で大きく不均衡） | [[sources/biomedclip]] |
| RSNA Pneumonia | NIH の正面像胸部 X 線 約 3 万枚、肺炎の二値 | [[sources/biomedclip]] |
| METER / QCR / MAML | 医療 VQA の枠組み（co-attention 融合 / 条件付き推論 / 視覚のみ事前学習） | [[sources/biomedclip]] |
| CheXbert | 放射線レポートから所見ラベルを自動抽出する分類器 | [[sources/biomedclip]] |
| BioViL / GLoRIA / ConVIRT / LoVT | 胸部 X 線特化の視覚–言語事前学習手法 | [[sources/biomedclip]] |
| PLIP | 医療 Twitter の OpenPath（約 20.8 万対）で訓練された病理特化 CLIP | [[sources/biomedclip]] |
| PubMedCLIP | ROCO（放射線 8 万対）で CLIP を微調整。⚠**汎用 CLIP より悪い** | [[sources/biomedclip]] |
| MedCLIP | BioClinicalBERT + Swin、MIMIC-CXR / CheXpert で微調整 | [[sources/biomedclip]] |
| Med-Flamingo | OpenFlamingo-9B ベースの医療 few-shot 学習器 | [[sources/biomedclip]] |
| LLaVA-Med | **BiomedCLIP を視覚エンコーダに使う**指示追従モデル | [[sources/biomedclip]] |
| Med-PaLM M | Google の 562B マルチモーダル医療モデル | [[sources/biomedclip]] / [[sources/medgemma]] |
| ROCO / ARCH | 放射線 8.8 万対 / 病理 7,562 対の医療画像–テキストデータセット | [[entities/pmc-15m]] |
| 正の転移（positive transfer） | 別領域のデータでの事前学習が当該領域の性能を上げる現象 | [[sources/biomedclip]] |
| ELR / ELR+ | Early-Learning Regularization（早期学習正則化）/ その拡張版 | [[sources/elr]] / [[concepts/learning-with-noisy-labels]] |
| 早期学習（early learning） | ノイズラベル訓練でモデルがまずクリーンなラベルに適合する初期段階 | [[concepts/learning-with-noisy-labels]] |
| 暗記（memorization） | その後モデルが誤ったラベルの方を覚え込む段階 | [[concepts/learning-with-noisy-labels]] |
| temporal ensembling | モデル出力の移動平均を目標とする半教師あり学習の技法 | [[sources/elr]] |
| 重み平均化（weight averaging） | モデルの重みの移動平均を取る技法（Mean Teacher 由来） | [[sources/elr]] |
| 確証バイアス（confirmation bias） | 自己訓練でモデルが自分の誤った予測を強化してしまう現象 | [[sources/elr]] |
| 対称ノイズ / 非対称ノイズ | ラベルが一様に反転する / クラス依存で反転する（より現実的） | [[sources/elr]] |
| 詐称者クラス（impostor class） | 誤ったラベルが指しているクラス | [[sources/elr]] |
| サンプル選択 / ラベル修正 / ロバスト損失 / 損失補正 | ノイズラベル対処の 4 類型（+ 正則化で 5 類型） | [[concepts/learning-with-noisy-labels]] |
| Co-teaching / Co-teaching+ | 2 つのネットワークが互いに損失の小さい事例を選び合う手法 | [[sources/elr]] |
| DivideMix | ノイズラベル問題を**半教師あり学習として定式化**する手法 | [[sources/elr]] |
| GCE / SL | Generalized Cross Entropy / Symmetric Cross Entropy（ロバスト損失） | [[sources/elr]] |
| SELFIE / PENCIL / Joint-Optim | ラベル修正系の手法 | [[sources/elr]] |
| Clothing1M | オンラインショップ由来の実世界ノイズラベルデータ（100 万枚、ノイズ率 38.5% と推定） | [[sources/elr]] |
| WebVision / mini WebVision | Web 由来の実世界ノイズラベルデータ | [[sources/elr]] |
| Schläfli の定理 | n 点を p 次元で線形分離できる分割の数を数える古典的結果 | [[sources/elr]]（定理 6 の鍵） |
| 一般の位置（general position） | 点集合が低次元のアフィン部分空間に縮退していない状態 | [[sources/elr]] |
| o_P(1) / 高確率で | 確率 0 に収束する確率量 / n,p→∞ で確率 1−o(1) | [[sources/elr]] |
| PreActResNet-18 | Pre-activation ResNet-18（ELR+ が CIFAR で使用） | [[sources/elr]] |
| RDNet | Revitalized DenseNet（DenseNet の近代化版） | [[entities/rdnet]] / [[sources/rdnet]] |
| DenseNet | Densely Connected Convolutional Networks（連結ショートカットの原典） | [[concepts/skip-connection]] |
| 加算ショートカット / additive shortcut | X + f(XW)。ResNet 以降の標準 | [[concepts/skip-connection]] |
| 連結ショートカット / concatenation | [X, f(XW)]。チャネル方向に繋ぐ。DenseNet 系 | [[concepts/skip-connection]] |
| GR（Growth Rate、成長率） | 1 つの特徴混合器が出力し連結されるチャネル数 | [[sources/rdnet]] |
| ER（Expansion Ratio、拡張率） | inverted bottleneck の中間次元の倍率。**DenseNet は GR に、RDNet は入力次元に掛ける** | [[sources/rdnet]] |
| 遷移層（transition layer） | チャネル数を減らす層。RDNet では**ダウンサンプリングも兼ねる** | [[sources/rdnet]] |
| 特徴混合器 / 混合ブロック | RDNet の構築ブロック（出力が GR 次元）/ 特徴混合器 ×3 + 遷移層 ×1 | [[sources/rdnet]] |
| 特徴の再利用（feature reuse） | 前の層の特徴をそのまま後段に渡すこと。DenseNet の中心概念 | [[concepts/skip-connection]] |
| RandNet | RDNet のパイロット研究で生成されるランダムネットワーク（空間 A〜E） | [[sources/rdnet]] |
| PreNorm / PostNorm | 正規化とショートカットの位置関係によるブロックの分類 | [[sources/rdnet]] |
| CKA | Centered Kernel Alignment（層の表現同士の類似度を測る指標） | [[sources/rdnet]] |
| Stochastic Depth | 訓練時に層を確率的にスキップする正則化 | [[sources/rdnet]] / [[sources/elr]] |
| RSB-ResNet | ResNet Strikes Back（現代的レシピで再訓練された ResNet） | [[sources/rdnet]] |
| DeiT-III | ViT の訓練レシピを改善した系譜 | [[sources/rdnet]] |
| HorNet / VAN / NAT / SMT / MogaNet / BiFormer / SLaK / RevCol | 2022〜2024 年の最新アーキテクチャ群（本 wiki 未取り込み） | [[sources/rdnet]] |
| iNaturalist-2018 / 2019 | 長尾分布の細粒度分類ベンチマーク | [[sources/rdnet]] |
| ImageNet-V2 / -A / -R / -Sketch / ObjectNet | 分布外（OOD）頑健性のベンチマーク | [[sources/rdnet]] |
| IS / FID | Inception Score / Fréchet Inception Distance（生成モデルの評価指標） | [[sources/rdnet]] |
| WGAN | Wasserstein GAN（RDNet が Appendix 0.G で生成器を連結型に置換） | [[sources/rdnet]] |
| InceptionNeXt | ConvNeXt の 7×7 depthwise を 4 分岐に分解した CNN バックボーン | [[entities/inceptionnext]] / [[sources/inceptionnext]] |
| Inception depthwise convolution | 大カーネル depthwise を 3×3 / 1×k / k×1 / 恒等の 4 分岐に分解したもの | [[sources/inceptionnext]] |
| MetaFormer | トークン混合器と MLP に**それぞれショートカットを持つ**（計 2 本）抽象ブロック | [[sources/inceptionnext]]（原典未取り込み ⚠） |
| MetaNeXt | MetaFormer の 2 本のショートカットを**1 本に統合**した抽象ブロック。ConvNeXt はその具体化 | [[sources/inceptionnext]] |
| トークン混合器（token mixer） | 空間方向の情報を混ぜる部品。ConvNeXt では DWConv、ViT では自己注意 | [[sources/inceptionnext]] |
| 帯状カーネル（band kernel） | $1\times k$ または $k\times 1$ の細長い畳み込みカーネル | [[sources/inceptionnext]] |
| 畳み込み分岐比（convolution branch ratio） | Inception depthwise で畳み込みを通すチャネルの割合（既定 1/8 ×3 分岐） | [[sources/inceptionnext]] |
| メモリアクセスコスト（memory access cost） | データの読み書きに要するコスト。**FLOPs に現れないが実速度を支配しうる** | [[sources/inceptionnext]] |
| スループット（throughput） | 単位時間あたりの処理枚数（img/s）。**訓練と推論で別に測る** | [[sources/inceptionnext]] / [[sources/nfnet]] |
| 等方的（isotropic）アーキテクチャ | ViT のように 1 ステージで解像度を落とさない構成 | [[sources/inceptionnext]] / [[sources/convnext]] |
| TF32 / FP32 | NVIDIA GPU の数値形式。TF32 は Ampere 以降で行列演算を高速化 | [[sources/inceptionnext]] |
| Channel First / Channel Last | テンソルのメモリレイアウト（NCHW / NHWC）。実速度に影響する | [[sources/inceptionnext]] |
| ShuffleNet V2 | 「全チャネルを畳み込みに通す必要はない」という知見の出どころ | [[sources/inceptionnext]]（原典未取り込み ⚠） |
| PoolFormer | MetaFormer のトークン混合器を単純なプーリングにしたモデル | [[sources/inceptionnext]]（原典未取り込み ⚠） |
| FocalNet | InceptionNeXt-T と同点（82.3）の同時期 CNN | [[sources/inceptionnext]]（原典未取り込み ⚠） |
| UperNet / Semantic FPN | セマンティックセグメンテーションのヘッド | [[sources/inceptionnext]] / [[sources/convnext]] |
| Grad-CAM | 勾配を使って CNN の注目領域を可視化する手法 | [[sources/inceptionnext]] |
| InceptionResNetV2 | WebVision の標準バックボーン | [[sources/elr]] |
| L2RW | Learning to Reweight（事例の重み付けの学習） | [[sources/l2rw]] / [[concepts/example-reweighting]] |
| bilevel 最適化 | 二重レベル最適化。内側の最適化の解を外側の目的関数が参照する入れ子構造 | [[sources/l2rw]] |
| backward-on-backward | 逆伝播で作った勾配のグラフをもう一度逆伝播すること（2 階自動微分） | [[sources/l2rw]] |
| MAML | Model-Agnostic Meta-Learning（Finn et al., 2017） | [[sources/l2rw]]（比較対象、原典未取り込み） |
| UniformFlip | 任意のクラスへ一様にラベルが反転するノイズ設定 | [[sources/l2rw]] |
| BackgroundFlip | 全クラスが単一の背景クラスへ反転するノイズ設定（不均衡 + ノイズの複合） | [[sources/l2rw]] |
| MentorNet | 損失値の系列を RNN に入れて事例の重みを出力するメタ学習モデル | [[sources/l2rw]]（比較対象） |
| Reed bootstrapping | 訓練目標をモデル予測と与えられたラベルの凸結合にする手法 | [[sources/l2rw]]（比較対象） |
| S-Model | 分類出力の後にノイズ遷移行列をモデル化する softmax 層を足す手法 | [[sources/l2rw]]（比較対象） |
| self-paced learning | 損失の小さい事例から順に学習させる手法（easy 優先） | [[concepts/example-reweighting]] |
| hard negative mining | 損失の大きい事例だけを選んで学習に使う手法（hard 優先） | [[concepts/example-reweighting]] / [[concepts/object-detection]] |
| hyper-validation set | ベースラインのハイパラ調整用に訓練セットから分割した集合（同じノイズで破損させる） | [[sources/l2rw]] |
| ES / FT | Early Stopping（早期停止）/ Fine-Tuning（ファインチューニング） | [[sources/l2rw]] |
| Lipschitz 平滑 | 勾配の変化が入力の変化に比例して抑えられる性質。収束証明の標準的仮定 | [[sources/l2rw]] |
| σ-有界勾配 | 勾配のノルムが σ 以下に抑えられること | [[sources/l2rw]] |
| LeNet | 1998 年の古典的な小規模 CNN。MNIST の標準ベースライン | [[concepts/convolutional-neural-network]] / [[sources/l2rw]] |
| LARS | Layer-wise Adaptive Rate Scaling。AGC が「緩和版」と位置づける正規化オプティマイザ | [[sources/nfnet]] |
| SE-ResNeXt-D | Squeeze-Excitation + ResNeXt + D 型ダウンサンプリング。NFNet の出発点 | [[sources/nfnet]] |
